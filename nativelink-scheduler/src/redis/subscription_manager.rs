...
use fred::types::{KeyspaceEvent, Message};
use nativelink_util::action_messages::OperationId;
use async_lock::Mutex;

use crate::awaited_action_db::{
    AwaitedAction, AwaitedActionSubscriber
};
...
impl RedisOperationSubscribersImpl {
    ...
    pub fn get_operation_sender(&mut self, operation_id: &OperationId) -> Option<watch::Sender<AwaitedAction>> {
    ...
    pub fn set_operation_sender(&mut self, operation_id: &OperationId, tx: watch::Sender<AwaitedAction>) {
        self.tx_map.insert(operation_id.clone(), tx);
    }

    pub fn get_operation_subscriber(&mut self, operation_id: &OperationId) -> Result<RedisOperationSubscriber, Error> {
        let Some(tx) = self.get_operation_sender(operation_id) else {
            return Err(make_err!(nativelink_error::Code::NotFound, "Could not find sender for operation {operation_id}"))
        };
...
enum RedisEventReciever {
    Keyspace(KeyspaceEvent),
    Message(Message)
}
...
impl RedisOperationSubscribers {
    pub fn new() -> Self {
        Self { inner: Arc::new(Mutex::new(RedisOperationSubscribersImpl::new())) }
    }


    pub async fn get_operation_sender(&self, operation_id: &OperationId) -> Option<watch::Sender<AwaitedAction>> {
    ...
    pub async fn set_operation_sender(&self, operation_id: &OperationId, tx: watch::Sender<AwaitedAction>) {
        let mut inner = self.inner.lock().await;
        inner.set_operation_sender(operation_id, tx)
    }

    pub async fn get_operation_subscriber(&self, operation_id: &OperationId) -> Result<RedisOperationSubscriber, Error> {
        let mut inner = self.inner.lock().await;
        inner.get_operation_subscriber(operation_id)
    }

    pub async fn get_operation_subscribers(&self, operation_ids: &[OperationId]) -> Vec<Result<RedisOperationSubscriber, Error>> {
        let mut inner = self.inner.lock().await;
        operation_ids.iter().map(|id| {
            inner.get_operation_subscriber(id)
        }).collect()
    }
}
