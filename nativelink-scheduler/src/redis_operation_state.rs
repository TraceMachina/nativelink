...
impl<T: ConnectionLike + Unpin + Clone + Send + Sync + 'static> RedisStateManager<T> {
        ...
        V::Err: Display,
    {
        ...
        let value_res: Result<Vec<V>, Error> = raw_values
            .into_iter()
            .map(|s| {
                V::from_str(&s).map_err(|e| {
                    make_input_err!("Error decoding list value {s}: {}", e.to_string())
                })
            })
            .collect();
    ...
    async fn inner_filter_operations(
        ...
        for operation in existing_operations.values() {
            if matches_filter(operation, &filter) {
                let operation_key =
                    build_operations_key(&operation.operation_id.to_string()).into();
                let store_subscription = self.store.clone().subscribe(operation_key).await;
