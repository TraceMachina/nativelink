...
impl RedisOperation {
    ...
    pub fn matches_filter(&self, filter: &OperationFilter) -> bool {
        ...
        let matches_last_update = filter.worker_update_before.map_or(true, |before| {
...
impl<T: ConnectionLike + Unpin + Clone + Send + Sync + 'static> RedisStateManager<T> {
    pub fn new(store: Arc<RedisStore<T>>, max_job_retries: usize) -> Self {
        Self {
            store,
            max_job_retries,
        }
    ...
    async fn inner_update_operation(
        ...
        let store = self.store.as_store_driver_pin();
        let key = build_operations_key(operation_id);
        ...
        let stage = match action_stage_result {
            Ok(stage) => RedisOperationStage::try_from(stage)
                .err_tip(|| "In RedisStateManager::inner_update_operation")?,
            Err(err) => {
                ...
                if operation.attempts > self.max_job_retries {
                    let error = Some(err.clone().merge(make_err!(
                        Code::Internal,
                        "Job cancelled because it attempted to execute too many times and failed"
                    )));
                    // Set the error field.
                    operation.last_error.clone_from(&error);
                    RedisOperationStage::Completed(ActionResult {
                        execution_metadata: ExecutionMetadata {
                            worker: worker_id.map_or_else(String::default, |v| v.to_string()),
                    ...
                    RedisOperationStage::Queued
                }
            }
        };
        // Set the appropriate operation stage and assign or unassign WorkerId.
        operation.stage = stage;
        operation.worker_id = worker_id.copied();
        store
            .update_oneshot(key.into(), operation.as_json().into())
            .await
    }

    async fn inner_assign_operation(
        &self,
        operation_id: &OperationId,
...
impl MatchingEngineStateManager for RedisStateManager {
    ...
    async fn assign_operation(
        ...
        self.inner_assign_operation(operation_id, worker_id_or_reason_for_unsassign)
            .await
    }
