...
use nativelink_store::ac_utils::{get_and_decode_digest, get_digests_and_output_dirs};
...
async fn validate_outputs_exist(
    ...
    let Ok((_, mut digest_infos, mut tree_digests)) =
        get_digests_and_output_dirs(action_result.clone())
    else {
        return false;
    };
    digest_infos.append(&mut tree_digests);
