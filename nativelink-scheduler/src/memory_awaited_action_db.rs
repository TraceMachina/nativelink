...
// TODO!(rename)
pub(crate) struct MatchingEngineActionStateResult<T: AwaitedActionSubscriber> {
...
/// [`AwaitedActionDbImpl::client_operation_to_awaited_action`], it must
...
pub struct AwaitedActionDbImpl {
    ...
    operation_drop_tx: mpsc::UnboundedSender<ActionEvent>,
}
...
impl AwaitedActionDbImpl {
    ...
    async fn handle_action_events(
        &mut self,
        operation_ids: impl IntoIterator<Item = ActionEvent>,
    ) -> NoEarlyReturn {
        for drop_action in operation_ids.into_iter() {
            match drop_action {
                ActionEvent::ClientDroppedOperation(operation_id) => {
                    ...
                    let sort_map_for_state = Self::get_sort_map_for_state(
                        &mut self.sorted_action_info_hash_keys,
                        &awaited_action.state().stage,
                    );

                    let maybe_sorted_awaited_action =
                        sort_map_for_state.take(&SortedAwaitedAction {
                            sort_key,
    ...
    fn get_awaited_actions_range<'a, 'b>(
        &'a self,
        range: impl RangeBounds<OperationId> + 'b,
    ) -> impl Iterator<Item = (&'a OperationId, MemoryAwaitedActionSubscriber)> + 'a {
        self.operation_id_to_awaited_action
            .range(range)
            .map(|(operation_id, tx)| {
                (
    ...
    fn get_sort_map_for_state<'a>(
        maps: &'a mut SortedAwaitedActions,
        state: &'_ ActionStage,
    ) -> &'a mut BTreeSet<SortedAwaitedAction> {
        match state {
            ActionStage::Unknown => &mut maps.unknown,
            ActionStage::CacheCheck => &mut maps.cache_check,
            ActionStage::Queued => &mut maps.queued,
            ActionStage::Executing => &mut maps.executing,
            ActionStage::Completed(_) => &mut maps.completed,
            ActionStage::CompletedFromCache(_) => &mut maps.completed,
        }
    }

    fn insert_sort_map_for_stage(
        maps: &mut SortedAwaitedActions,
        stage: &ActionStage,
        sorted_awaited_action: SortedAwaitedAction,
    ) -> Result<(), Error> {
        let newly_inserted = match stage {
            ActionStage::Unknown => maps.unknown.insert(sorted_awaited_action.clone()),
            ActionStage::CacheCheck => maps.cache_check.insert(sorted_awaited_action.clone()),
            ActionStage::Queued => maps.queued.insert(sorted_awaited_action.clone()),
            ActionStage::Executing => maps.executing.insert(sorted_awaited_action.clone()),
            ActionStage::Completed(_) => maps.completed.insert(sorted_awaited_action.clone()),
            ActionStage::CompletedFromCache(_) => {
                maps.completed.insert(sorted_awaited_action.clone())
            }
        };
        if !newly_inserted {
            return Err(make_err!(
    ...
    fn set_awaited_action(&mut self, new_awaited_action: AwaitedAction) -> Result<(), Error> {
        let tx = self
            ...
            .ok_or_else(|| {
                make_err!(
                    Code::Internal,
                    "OperationId does not exist in map in AwaitedActionDb::set_awaited_action"
                )
            })?;

        let old_awaited_action = tx.borrow();
        let old_sort_key = old_awaited_action.sort_key();
        ...
        if !is_same_stage {
            // Do not allow subscribe if the action is already completed,
            ...
            if new_awaited_action.state().stage.is_finished() {
                match &new_awaited_action.action_info().unique_qualifier {
                    ...
                    ActionUniqueQualifier::Uncachable(_action_key) => {
                        // Nothing to do, it won't be in our map here.
                    }
                }
            }
            let btree = Self::get_sort_map_for_state(
                &mut self.sorted_action_info_hash_keys,
                &old_awaited_action.state().stage,
            );
            drop(old_awaited_action);
            ...
            .err_tip(|| "In AwaitedActionDb::set_awaited_action")?;
        } else {
            drop(old_awaited_action);
        }
...
pub struct MemoryAwaitedActionDb {
    ...
    _cleanup_operations_spawn: JoinHandleDropGuard<()>,
}

impl MemoryAwaitedActionDb {
    pub fn new(eviction_config: &EvictionPolicy) -> Self {
        let (operation_drop_tx, mut operation_drop_rx) = mpsc::unbounded_channel();
        ...
        Self {
            inner,
            _cleanup_operations_spawn: spawn!("cleanup_awaited_actions", async move {
...
impl AwaitedActionDb for MemoryAwaitedActionDb {
    ...
    async fn get_awaited_action_by_id<'a>(
        &'a self,
    ...
    async fn get_by_operation_id<'a>(
        &'a self,
    ...
    async fn get_range(
        ...
        ChunkedStream::new(start, end, move |start, end, mut output| async move {
            ...
            let iterator = inner.get_range(state, (start.as_ref(), end.as_ref()));
            ...
            if desc {
                for result in iterator.rev() {
                    let (sorted_awaited_action, item) =
                        result.err_tip(|| "In AwaitedActionDb::get_range")?;
                    output.push_back(item);
    ...
    async fn set_awaited_action(&self, new_awaited_action: AwaitedAction) -> Result<(), Error> {
        self.inner
            .lock()
            .await
            .set_awaited_action(new_awaited_action)
    }
