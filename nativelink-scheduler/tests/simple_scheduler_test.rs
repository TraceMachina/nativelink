...
///
/// Pre-fix, `UpdateOperationType::UpdateWithDisconnect` was handled by
/// `simple_scheduler_state_manager::inner_update_operation` as an
/// unconditional `ActionStage::Queued` with no `attempts` increment,
/// so `max_job_retries` never tripped. The customer-visible failure
/// mode: a worker pod whose memory limit was below the peak working
/// set of its action set OOMKilled, the scheduler re-dispatched the
/// same action set to the freshly-respawned worker, that worker OOMed
/// the same way, and the loop continued indefinitely. Only Bazel's
/// client-side `--test_timeout` eventually broke the cycle, surfacing
/// to the user as TIMEOUT or NO STATUS — neither pointing at the
/// cluster.
///
/// This test mirrors `worker_retries_on_internal_error_and_fails_test`
/// but uses `UpdateWithDisconnect` instead of `UpdateWithError`. With
/// `max_job_retries=1`, the first disconnect must re-queue the action
/// (attempts=1, under threshold) and the second disconnect must
/// transition to `Completed` with an error message that explicitly
/// names the worker-disconnect path so operators have something to
/// grep for instead of chasing ambiguous TIMEOUT/NO STATUS in Bazel
/// output.
#[nativelink_test]
