...
//     let insert_timestamp1 = make_system_time(1);
//     let (client1_id, mut client1_rx) = setup_action(
//         &scheduler,
//         action_digest1,
...
//     )
//     .await?;
//     let operation_id1 = scheduler.find_existing_action(&client1_id).await?.unwrap().borrow().id.clone();
//     let insert_timestamp2 = make_system_time(1);
//     let (client2_id, mut client2_rx) = setup_action(
//         &scheduler,
//         action_digest2,
//         platform_properties,
//         insert_timestamp2,
//     )
//     .await?;
//     let operation_id2 = scheduler.find_existing_action(&client2_id).await?.unwrap().borrow().id.clone();
//
//     match rx_from_worker.recv().await.unwrap().update {
//         Some(update_for_worker::Update::StartAction(_)) => { /* Success */ }
//         v => panic!("Expected StartAction, got : {v:?}"),
