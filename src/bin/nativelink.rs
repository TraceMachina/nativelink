...
async fn inner_main(cfg: CasConfig, server_start_timestamp: u64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (server_cfg, connected_clients_mux) in servers_and_clients {
        ...
        let health_registery_status = health_registry.clone();
                ...
                axum::routing::get(move || async move {
                    ...
                    spawn_blocking(move || {
                        let response = futures::executor::block_on(async {
                            ...
                            let mut health_registery_status_guard = health_registery_status.lock().await;
                            let mut indicators = health_registery_status_guard.get_health_status_report();
                            while let Some(indicator) = indicators.next().await {
                                buf.push_str(&format!("{:?}\n", indicator));
                            }
