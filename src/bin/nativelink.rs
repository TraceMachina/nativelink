...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[allow(clippy::disallowed_methods)]
    {
        ...
        let (shutdown_tx, _) = broadcast::channel::<Arc<oneshot::Sender<()>>>(16);
        let shutdown_tx_clone = shutdown_tx.clone();

        // Create a oneshot channel for triggering the complete signal
        ...
        #[cfg(target_family = "unix")]
        {
            ...
            runtime.spawn(async move {
                ...
                if shutdown_tx_clone.send(complete_tx).is_err() {
                    eprintln!("Failed to send shutdown signal");
                }
                if let Ok(()) = complete_rx.await {
                } else {
                    eprintln!("Successfully shut down.");
