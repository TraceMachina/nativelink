    ...
    for (server_cfg, connected_clients_mux) in servers_and_clients {
        ...
        root_futures.push(Box::pin(async move {
            loop {
                ...
                OriginContext::builder().background_spawn(
                    error_span!(
                        target: "nativelink::services",
                        "http_connection",
                        ?remote_addr,
                ...
                background_spawn!(
                    ...
                    fut: async move {
                        ...
                        let ignore_err_handler = |err: hyper::Error| {
                            use std::error::Error;
                        ...
                        let http_svc_fut = if let Some(tls_acceptor) = maybe_tls_acceptor {
                            ...
                            http.serve_connection(tls_stream, svc).fuse()  //.left_future()
                        } else {
                            todo!();
                            // http.serve_connection(tcp_stream, svc).right_future()
                        };
                        tokio::pin!(http_svc_fut);
                        let mut futures = FuturesUnordered::new();
                        loop {
                            tokio::select! {
                                maybe_new_future = rx.recv() => {
                                    match maybe_new_future {
                                        Some(new_future) => futures.push(new_future),
                                        None => {
                                            event!(
                                                target: "nativelink::services",
                                                Level::DEBUG,
                                ...
                                result = &mut http_svc_fut => {
                                    if let Err(err) = result.map_err(ignore_err_handler) {
                                        event!(
                                            target: "nativelink::services",
                                            Level::ERROR,
                                            ?err,
