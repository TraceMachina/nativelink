...
macro_rules! make_input_err {
    ($($arg:tt)+) => {{
        $crate::make_err!($crate::error::Code::InvalidArgument, $($arg)+)
    }};
}
...
macro_rules! error_if {
    ($cond:expr, $($arg:tt)+) => {{
        if $cond {
            Err($crate::make_err!($crate::error::Code::InvalidArgument, $($arg)+))?;
        }
    }};
}

// TODO(aaronmondal): Macros are exported at crate root, not module root.
//                    Reexport the macros so that they're available as under the
//                    more intuitive crate::error path. We might want to rework
//                    this entire logic at some point.
pub use crate::{error_if, make_err, make_input_err};
