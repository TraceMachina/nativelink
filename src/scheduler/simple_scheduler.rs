...
use crate::config::schedulers::WorkerAllocationStrategy;
use crate::error::{error_if, make_err, make_input_err, Code, Error, ResultExt};
...
use crate::scheduler::platform_property_manager::{PlatformPropertyManager, PlatformPropertyValue};
use crate::scheduler::worker::{Worker, WorkerId, WorkerTimestamp, WorkerUpdate};
use crate::scheduler::worker_scheduler::WorkerScheduler;
use crate::util::common::log;
...
impl SimpleScheduler {
    #[inline]
    #[must_use]
    pub fn new(scheduler_cfg: &crate::config::schedulers::SimpleScheduler) -> Self {
    ...
    pub fn new_with_callback<Fut: Future<Output = ()> + Send, F: Fn() -> Fut + Send + Sync + 'static>(
        scheduler_cfg: &crate::config::schedulers::SimpleScheduler,
