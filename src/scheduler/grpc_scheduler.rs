...
use crate::scheduler::action_messages::{ActionInfo, ActionInfoHashKey, ActionState, DEFAULT_EXECUTION_PRIORITY};
...
impl GrpcScheduler {
    pub fn new(config: &crate::config::schedulers::GrpcScheduler) -> Result<Self, Error> {
    ...
    pub fn new_with_jitter(
        config: &crate::config::schedulers::GrpcScheduler,
        jitter_fn: Box<dyn Fn(Duration) -> Duration + Send + Sync>,
    ) -> Result<Self, Error> {
        ...
        self.perform_request(instance_name, |instance_name| async move {
            ...
            let platform_property_manager = Arc::new(PlatformPropertyManager::new(
                capabilities
                    ...
                    .iter()
                    .map(|property| (property.clone(), crate::config::schedulers::PropertyType::Exact))
                    .collect(),
            ));
