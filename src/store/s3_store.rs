...
use rusoto_core::{ByteStream, HttpClient, HttpDispatchError, RusotoError};
use rusoto_s3::{
...
use crate::util::common::{log, DigestInfo, JoinHandleDropGuard};
...
impl S3Store {
    pub fn new(config: &crate::config::stores::S3Store) -> Result<Self, Error> {
    ...
    pub fn new_with_client_and_jitter(
        config: &crate::config::stores::S3Store,
        s3_client: S3Client,
        jitter_fn: Box<dyn Fn(Duration) -> Duration + Send + Sync>,
    ) -> Result<Self, Error> {
        Ok(Self {
