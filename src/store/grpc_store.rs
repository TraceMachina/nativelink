...
use crate::error::{error_if, make_input_err, Error, ResultExt};
use crate::store::ac_utils::ESTIMATED_DIGEST_SIZE;
use crate::store::traits::{StoreTrait, UploadSizeInfo};
use crate::util::buf_channel::{DropCloserReadHalf, DropCloserWriteHalf};
use crate::util::common::{log, DigestInfo};
use crate::util::retry::{ExponentialBackoff, Retrier, RetryResult};
use crate::util::write_request_stream_wrapper::WriteRequestStreamWrapper;
...
pub struct GrpcStore {
    ...
    store_type: crate::config::stores::StoreType,
    jitter_fn: Box<dyn Fn(Duration) -> Duration + Send + Sync>,
    retry: crate::config::stores::Retry,
    retrier: Retrier,
}

impl GrpcStore {
    pub async fn new(config: &crate::config::stores::GrpcStore) -> Result<Self, Error> {
    ...
    pub async fn new_with_jitter(
        config: &crate::config::stores::GrpcStore,
        jitter_fn: Box<dyn Fn(Duration) -> Duration + Send + Sync>,
    ) -> Result<Self, Error> {
    ...
    pub async fn find_missing_blobs(
        ...
        error_if!(
            matches!(self.store_type, crate::config::stores::StoreType::AC),
...
impl StoreTrait for GrpcStore {
    ...
    async fn has_with_results(
        ...
        if matches!(self.store_type, crate::config::stores::StoreType::AC) {
            digests
                .iter()
