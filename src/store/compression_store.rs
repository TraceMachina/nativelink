...
use crate::error::{error_if, make_err, Code, Error, ResultExt};
...
use crate::util::common::{DigestInfo, JoinHandleDropGuard};
...
/// This is a partial mirror of crate::config::stores::Lz4Config.
...
pub struct CompressionStore {
    ...
    config: crate::config::stores::Lz4Config,
    bincode_options: BincodeOptions,
}

impl CompressionStore {
    pub fn new(
        compression_config: crate::config::stores::CompressionStore,
        inner_store: Arc<dyn StoreTrait>,
    ) -> Result<Self, Error> {
        let lz4_config = match compression_config.compression_algorithm {
            crate::config::stores::CompressionAlgorithm::LZ4(mut lz4_config) => {
