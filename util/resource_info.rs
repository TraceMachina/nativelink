...
impl<'a> ResourceInfo<'a> {
    pub fn new(resource_name: &'a str) -> Result<ResourceInfo<'a>, Error> {
        ...
        let Some(blobs_position) = parts[..parts.len() - 2].iter().rev().position(|part| *part == "blobs" || *part == "compressed-blobs") else {
            return Err(make_input_err!("{}", ERROR_MSG));
        };
        // The position was on the reversed iterator, so swap it back.
        let blobs_position = parts.len() - blobs_position - 3;
