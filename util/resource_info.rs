...
impl<'a> ResourceInfo<'a> {
    pub fn new(resource_name: &'a str, is_upload: bool) -> Result<ResourceInfo<'a>, Error> {
        ...
        let end_state = recrusive_parse(&mut rparts, &mut output, State::Unknown, &mut end_bytes_processed)?;
        error_if!(
        ...
        let parsed_pos = resource_name.len() - end_bytes_processed;
        let mut beginning_part = &resource_name[..parsed_pos];
        if !beginning_part.is_empty() {
            // If we are not empty, we definitly have a trailign slash.
            beginning_part = &beginning_part[..(beginning_part.len() - SLASH_SIZE)];
        }
...
// populating the output struct. This recrusive function utializes the stack to
// temporarly hold the reference to the previous item reducing the need for
// a heap allocation.
fn recrusive_parse<'a>(
    ...
    if state == State::Unknown {
        ...
        state = recrusive_parse(rparts, output, state, bytes_processed)?;
    }
