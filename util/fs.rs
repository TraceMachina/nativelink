...
static OPEN_FILE_LOCKS: Mutex<Option<HashMap<String, Arc<RwLock<()>>>>> = parking_lot::const_mutex(None);

fn get_lock(path: impl AsRef<Path>) -> Arc<RwLock<()>> {
    let mut locks = OPEN_FILE_LOCKS.lock();
...
pub async fn open_file_read_locked(path: impl AsRef<Path> + std::fmt::Debug) -> Result<FileSlot<'static>, Error> {
    ...
    let _file_permit = lock.read().await;
    Ok(FileSlot {
        _permit: permit,
...
pub async fn hard_link_src_locked(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> Result<(), Error> {
    ...
    let lock = get_lock(&src);
    let _src_file_permit = lock.read().await;
...
pub async fn rename_src_locked(from: impl AsRef<Path>, to: impl AsRef<Path>) -> Result<(), Error> {
    let _permit = take_open_semaphore().await?;
    let lock = get_lock(&from);
    let _from_file_permit = lock.write().await;
...
pub async fn rename_dest_locked(from: impl AsRef<Path>, to: impl AsRef<Path>) -> Result<(), Error> {
    let _permit = take_open_semaphore().await?;
    let lock = get_lock(&to);
    let _to_file_permit = lock.write().await;
    tokio::fs::rename(from, to).await.map_err(|e| e.into())
}
...
pub async fn remove_file_locked(path: impl AsRef<Path>) -> Result<(), Error> {
    let _permit = take_open_semaphore().await?;
    let lock = get_lock(&path);
    let _file_permit = lock.write().await;
