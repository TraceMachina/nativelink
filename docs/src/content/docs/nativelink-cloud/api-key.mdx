...
For PRs we recommend checking in a read-only key, here's an example from our GitHub Action below:

:::note
You can create a read-only key in the settings page of the NativeLink Cloud app.
:::

```yaml
bazel test \
  ...
  --remote_header=x-nativelink-api-key=${{ github.ref == 'refs/heads/main' && secrets.NATIVELINK_COM_API_HEADER || 'YOUR_HARDCODED_READ_ONLY_KEY_HERE' }} \
  ...
  ${{ github.ref != 'refs/heads/main' && '--nogenerate_json_trace_profile --remote_upload_local_results=false \' || '' }}
  //...
```

For merge-to-main actions, use a Read/Write key stored in the secrets context.
