...
/// connection itself.
type IndexedChannel = Result<(ChannelIdentifier, Channel), (ChannelIdentifier, Error)>;
...
impl ConnectionManagerWorker {
    ...
    async fn connect_next(&mut self) {
        ...
        } else if let Some(connection_result) = self.connecting_channels.next().await {
    ...
    fn connect_endpoint(&mut self, endpoint_index: usize, connection_index: Option<usize>) {
        if let Some(endpoint) = self.endpoints.get_mut(endpoint_index) {
            ...
            if is_backoff {
                warn!("Connection {connection_index} failed to {endpoint:?}, reconnecting.");
            } else {
                info!("Connection {connection_index} to {endpoint:?} in error, reconnecting.");
            }
            let identifier = ChannelIdentifier {
                endpoint_index,
                connection_index,
            };
            let connection_stream = unfold(endpoint, move |endpoint| async move {
                match endpoint.connect().await {
                    Ok(channel) => Some((RetryResult::Ok(channel), endpoint)),
                    Err(err) => Some((
    ...
    fn maybe_available_connection(&mut self) {
        while self.available_connections > 0 && !self.waiting_connections.is_empty() {
