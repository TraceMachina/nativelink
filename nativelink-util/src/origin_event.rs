...
type OriginEventCollector = mpsc::Sender<OriginEvent>;
...
impl OriginEventContext<()> {
        ...
        T: OriginEventSource<U> + 'static,
    {
        ...
        async move {
            let parent_event_id =
                publish_origin_event_impl(event, None, &origin_event_collector).await;
            OriginEventContext {
...
async fn publish_origin_event_impl(
    event: Event,
    parent_event_id: Option<Uuid>,
    origin_event_collector: &OriginEventCollector,
) -> Uuid {
    let bazel_request_metadata = match ActiveOriginContext::get_value(&BAZEL_REQUEST_METDATA) {
        Ok(v) => v.map(|v| v.as_ref().clone()),
        _ => None,
    };

    let event_id = Uuid::now_v6(get_node_id());
    // Failing to send this event means that the receiver has been dropped.
    let _ = origin_event_collector
        .send(OriginEvent {
            version: ORIGIN_EVENT_VERSION,
            event_id: event_id.as_hyphenated().to_string(),
            parent_event_id: parent_event_id
                .map_or_else(String::new, |id| id.as_hyphenated().to_string()),
...
pub trait OriginEventSource<Source>: Sized {
    ...
    fn publish<'a>(&self, ctx: &'a OriginEventContextImpl) -> impl Future<Output = ()> + Send + 'a {
        ...
        async move {
            // We don't need the event_id here.
            let _ = publish_origin_event_impl(
                event,
