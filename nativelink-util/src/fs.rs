...
pub fn set_open_file_limit(desired_open_file_limit: usize) {
    ...
    // That's just the default in rlimit, can we give a better estimate?
    ...
    println!("first {new_open_file_limit}");
    // We only use 80% of the open file limit for permits from
    // OPEN_FILE_SEMAPHORE to give extra room for other file descriptors like
    // sockets, pipes, and other things.
    let reduced_open_file_limit = new_open_file_limit.saturating_sub(new_open_file_limit / 5);
    println!("second {reduced_open_file_limit}");
        ...
        < previous_open_file_limit
    {
        event!(
            Level::ERROR,
            "The amount of open files is larger than the reduced open file limit ({reduced_open_file_limit}).",
        );
    }
