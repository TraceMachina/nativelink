    ...
    T: Stream<Item = Result<WriteRequest, E>> + Unpin,
{
    ...
    // Note: It is important to ensure that this count is kept correct otherwise
    // first_msg may be sent more times than expected.  This is due to first_msg
    // being cloned to allow for retry and this invarient is held true by the
    // from() and reset() functions.
    message_count: usize,
    write_finished: bool,
}
    ...
    T: Stream<Item = Result<WriteRequest, E>> + Unpin,
{
    ...
    /// Note that calling this when is_retryable() returns `false` will result
    /// in the first poll_next() returning an error.
    ...
    T: Stream<Item = Result<WriteRequest, E>> + Unpin,
{
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let maybe_message = if self.message_count == 0 {
            self.first_msg
                .clone()
                .ok_or_else(|| make_input_err!("First message was lost in write stream wrapper"))
        } else {
