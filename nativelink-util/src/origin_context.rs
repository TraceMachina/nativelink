...
impl OriginContext {
    ...
    #[inline(always)]
    fn current() -> Option<Arc<OriginContext>> {
...
impl<T> ContextAwareFuture<T> {
    ...
    pub fn new_from_current(inner: Instrumented<T>) -> ContextAwareFuture<T> {
        match OriginContext::current() {
            ...
            None => panic!("OriginContext must be set"),
        }
    }

    #[must_use = "futures do nothing unless you `.await` or poll them"]
    #[inline(always)]
    fn new(context: Option<Arc<OriginContext>>, inner: Instrumented<T>) -> Self {
        Self {
...
impl<T: Future> Future for ContextAwareFuture<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let ctx = this.context.take().expect("Expected context to exist");
        let enter = ctx.enter();
