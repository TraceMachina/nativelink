...
impl HealthStatus {
    ...
    pub fn new_ok(component: &(impl HealthStatusIndicator + ?Sized), message: Message) -> Self {
        Self::Ok(component.struct_name(), message)
    }

    /// Make an initializing status.
    pub fn new_initializing(component: &(impl HealthStatusIndicator + ?Sized), message: Message) -> HealthStatus {
        Self::Initializing(component.struct_name(), message)
    }

    /// Make a warning status.
    pub fn new_warning(component: &(impl HealthStatusIndicator + ?Sized), message: Message) -> HealthStatus {
        Self::Warning(component.struct_name(), message)
    }

    /// Make a failed status.
    pub fn new_failed(component: &(impl HealthStatusIndicator + ?Sized), message: Message) -> HealthStatus {
