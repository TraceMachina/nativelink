...
pub enum HealthStatus {
    ...
    /// Warning status, used to signal or indicate component is in a warning state.
    /// This status is used to indicate a non-fatal issue with the component.
    Warning {
        struct_name: &'static StructName,
        message: Cow<'static, Message>,
    },
    /// Failed status, used to signal or indicate component is in a failed state.
    /// This status is used to indicate a fatal issue with the component,
...
/// Health registry that holds the health status indicators of a component.
#[derive(Default, Clone)]
pub struct HealthRegistry {
    indicators: Vec<(Cow<'static, str>, Arc<dyn HealthStatusIndicator>)>,
}

/// Reporter of the health status health registry.
