...
use nativelink_proto::build::bazel::remote::execution::v2::{platform::Property as ProtoProperty, Platform as ProtoPlatform};
...
impl RedisPlatformPropertyValue {
    pub fn new(name: String, value: String) -> Self {
        Self { name, value }
    }
}

impl From<RedisPlatformPropertyValue> for ProtoProperty {
    fn from(v: RedisPlatformPropertyValue) -> ProtoProperty {
        ProtoProperty { name: v.name, value: v.value }
    }
}
impl From<ProtoProperty> for RedisPlatformPropertyValue {
    fn from(property: ProtoProperty) -> Self {
        Self { name: property.name, value: property.value }
    }
}

impl From<RedisPlatformPropertyValue> for PlatformPropertyValue {
    fn from(prop: RedisPlatformPropertyValue) -> Self {
        Self::Unknown(prop.value)
    }
}

impl From<ProtoPlatform> for RedisPlatformProperties {
    fn from(value: ProtoPlatform) -> Self {
        RedisPlatformProperties {
            properties: Vec::from_iter(value.properties.iter().map(|v| v.to_owned().into()))
        }
    }
}
...
pub struct RedisPlatformProperties {
    pub properties: Vec<RedisPlatformPropertyValue>,
}

impl From<PlatformProperties> for RedisPlatformProperties {
    fn from(platform: PlatformProperties) -> Self {
        let properties: Vec<RedisPlatformPropertyValue> = platform.properties
            .iter()
            .map(|(name, property_value)| {
                let value = match property_value {
                    PlatformPropertyValue::Unknown(p)
                    | PlatformPropertyValue::Exact(p)
                    | PlatformPropertyValue::Priority(p) => p.to_owned(),
                    PlatformPropertyValue::Minimum(p) => p.to_string(),

                };
                RedisPlatformPropertyValue::new(name.to_string(), value)
            }).collect();
        Self {
            properties
        }
...
impl From<RedisPlatformProperties> for PlatformProperties {
    fn from(value: RedisPlatformProperties) -> Self {
        let properties: HashMap<String, PlatformPropertyValue> = HashMap::from_iter(
            value.properties.iter().map(|property| {
                (property.name.to_owned(), PlatformPropertyValue::from(property.to_owned()))
            }));
        Self {
            properties
        }
