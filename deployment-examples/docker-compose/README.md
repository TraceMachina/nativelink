...
- [`local-storage-cas-zstd.json5`](./local-storage-cas-zstd.json5) - Same, but the CAS keeps blobs as zstd at rest via `zstd_store` for byte-for-byte `--remote_cache_compression` passthrough (see the file's header comment for the dedicated-namespace and placement rules)
