...
resource "google_compute_region_autoscaler" "browser_autoscaler" {
  ...
  autoscaling_policy {
    ...
    load_balancing_utilization {
      target = google_compute_region_backend_service.browser_backend_service.id
    }
...
resource "google_compute_region_instance_group_manager" "browser_instance_group" {
  ...
  auto_healing_policies {
    health_check      = google_compute_region_health_check.browser_health_checker.id
...
resource "google_compute_region_instance_template" "browser_instance_template" {
  ...
  metadata = {
    ...
    native-link-hmac-secret-key = google_secret_manager_secret.browser_secret_manager_hmac_key.secret_id
  }
}

resource "google_compute_region_backend_service" "browser_backend_service" {
  ...
  health_checks = [
    google_compute_region_health_check.browser_health_checker.id,
  ]
...
resource "google_compute_region_health_check" "browser_health_checker" {
...
resource "google_compute_url_map" "browser_load_balancer" {
  ...
  default_service = google_compute_region_backend_service.browser_backend_service.id
}
