...
<Callout type="warn" title="A few wrappers only make sense one way round">
  ...
  - A `fast_slow` read can use `fast` without checking `slow`. With a writable
    slow tier, existence checks instead require presence in `slow`, including
    pending uploads. On `main` (not yet released), a `read_only` or `get` slow
    tier makes existence checks accept either tier. That establishes
    availability, not durability after the fast tier is lost.
...
<Accordions type="multiple">
  <Accordion title="Does fast_slow guarantee the slow tier has everything?" id="does-fast-slow-guarantee-the-slow-tier-has-everything">
    A read from `fast` does not establish presence in `slow`. With a writable
    slow tier, existence checks do require it, including pending uploads.
    If an artifact must survive a fast-tier wipe, make sure the write path
    covers both tiers deliberately. Read-only existence checks on `main`
    (not yet released) accept either tier and carry no durability guarantee.
    <SourceLink file="nativelink-store/src/fast_slow_store.rs" symbol="FastSlowStore" />
  </Accordion>
