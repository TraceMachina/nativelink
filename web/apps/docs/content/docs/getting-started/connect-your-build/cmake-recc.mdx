...
## 1. Start NativeLink [#1-start-nativelink]
...
<Tabs items={["macOS / Homebrew", "Linux from source (apt)"]}>
  ...
  <Tab value="Linux from source (apt)">
    ...
    sudo apt-get install -y cmake g++ gcc git googletest libabsl-dev \
      ...
      libssl-dev pkg-config uuid-dev nlohmann-json3-dev libcurl4-openssl-dev
    ```

    Then build BuildBox, which builds and installs `recc` with it:
    ...
    [BuildBox install docs](https://buildgrid.gitlab.io/buildbox/buildbox/installation.html)
    and the [packaged recc page](https://buildgrid.gitlab.io/buildbox/buildbox/recc-installation.html).
  </Tab>
</Tabs>

## 3. Create the example [#3-create-the-example]

Drop these two files into a fresh project directory and `cd` into it.
You can also copy them from
<SourceLink file="templates/cmake" dir symbol="templates/cmake/" />
in the repo.
...
## 4. Point recc at NativeLink [#4-point-recc-at-nativelink]
...
- `RECC_INSTANCE=main`: must match an `instance_name` in your NativeLink
  config. `basic_cas.json5` exposes both `""` and `"main"`; `recc`
  defaults to `""` unless it was built with a different
  `DEFAULT_RECC_INSTANCE`, so setting it explicitly removes the guesswork.
...
- `RECC_CACHE_UPLOAD_LOCAL_BUILD=1`: push the result of a local compile
  back up so the next build (or the next teammate) gets a hit. It only
  has an effect together with `RECC_CACHE_ONLY`.
...
## 6. Confirm the cache is live [#6-confirm-the-cache-is-live]
...
<Accordions type="multiple">
  ...
  <Accordion title="How do I share the cache with my team?" id="how-do-i-share-the-cache-with-my-team">
    ...
    `RECC_INSTANCE` identical across machines. NativeLink rejects
    requests for an instance name it does not serve, so a mismatch shows
    up as recc errors on stderr and every compile running locally.
  </Accordion>
</Accordions>

## Going further [#going-further]

- Point `RECC_SERVER` at a shared NativeLink deployment to share the
  cache across your team.
- Drop `RECC_CACHE_ONLY` once you have remote workers configured to
  actually offload compile work. See
  [Remote execution](/remote-execution).
- For PCH, LTO, and other gnarly C++ build features at scale, read
  [Reid Kleckner's writeup](https://reidkleckner.dev/posts/llvm-recc-nativelink/).
