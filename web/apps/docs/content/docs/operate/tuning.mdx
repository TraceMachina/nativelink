...
| Per-request object-store cost dominated by tiny blobs | `size_partitioning` | Split at ~64k–1MB | Small blobs go to a cheap fast store, large ones to the object store | Two stores to size instead of one |
...
| One backend at its throughput ceiling | `shard` | Add shards | Load spreads by digest across backends | Reshading changes which backend holds what; plan it as a migration |
| Read latency high but the working set is small | `fast_slow` hot tier | Size `fast` to the 24–48h working set | Hot reads served locally, cold reads fall through | `fast_slow` never verifies the slow tier has everything — see the warning on [production config](/operate/production-config) |
...
| Actions never dispatched to the right hardware | worker `platform_properties` | Add the property | The scheduler can route on it | Only works if the scheduler also lists it — see below |
...
| Signal you see | Knob | Direction | Expected effect | Cost / risk |
| --- | --- | --- | --- | --- |
| "Too many open files" under load | `global.max_open_files` | Up | More concurrent blobs in flight | Must be within the OS `ulimit -n`; the binary defaults to 24576 when unset |
| Redis connection saturation | `connection_pool_size` on `redis` | Up from the default of 3 | More concurrent Redis commands | More connections against the Redis server's own limit |
| Redis commands timing out | `command_timeout_ms` / `connection_timeout_ms` | Up | Fewer spurious failures on a loaded Redis | Slower failure detection. Use these, not the deprecated `response_timeout_s` / `connection_timeout_s` |
