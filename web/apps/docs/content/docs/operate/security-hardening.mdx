...
the flag list also gains `CLONE_NEWNS`, the worker's own root action
directory is masked so one action cannot see another's tree, and `/tmp` is
replaced with a private, empty tmpfs so actions cannot collide on, or leak
state through, predictable paths there (`isolate_tmp: false` keeps the host's
`/tmp` visible); the rest of the host filesystem stays visible.
