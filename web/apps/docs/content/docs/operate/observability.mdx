...
## Experimental input mounts [#experimental-input-mounts]

Workers emit `worker.input_mount.cache_hits` and `cache_misses` to show
whether selected directories are reused or rebuilt. `worker.input_mount.fallbacks`
counts actions using ordinary input setup despite configured mounts;
`worker.input_mount.prepare_failures` counts failed preparations retried without
mounts. These counters describe preparation, not successful command execution.
These names use OTLP notation; see [Prometheus naming conventions](/reference/metrics#how-an-instrument-becomes-a-prometheus-series) for the conversion.
