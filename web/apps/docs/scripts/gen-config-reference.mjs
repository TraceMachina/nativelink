...
//
// Usage:
//   node scripts/gen-config-reference.mjs            # all versions
//   node scripts/gen-config-reference.mjs main       # just main (fast iteration)
//   node scripts/gen-config-reference.mjs main v1.5.0
//
...
function resolveVersions(filter) {
  ...
  if (!filter || filter.length === 0) return versions;
  const wanted = new Set(filter);
  return versions.filter((v) => wanted.has(v.version));
}
...
function generateOne(v, sharedTargetDir) {
  ...
  if (v.isDev) {
    ...
    worktree = mkdtempSync(join(tmpdir(), `nl-cfg-${v.slug}-`));
...
function writeManifest(generated) {
  const entries = generated.map((v) => ({
    version: v.version,
    label: v.label,
    href: hrefFor(v),
...
function writeMeta() {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ title: "NativeLink config", pages: ["index"] }, null, 2)}\n`,
  );
}

function main() {
  const filter = process.argv.slice(2);
  const versions = resolveVersions(filter);
  ...
  for (const v of versions) {
    ...
    try {
      ...
      skipped.push({ version: v.version, error: String(err.message || err) });
  ...
  if (skipped.length) {
    ...
    for (const s of skipped) console.log(`  ${s.version}: ${s.error.split("\n")[0]}`);
  }
}
