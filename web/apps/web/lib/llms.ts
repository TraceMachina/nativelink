...
**Why we say migrate to Bazel.** We believe closed systems are dangerous. A
build system you cannot read is one you cannot audit, cannot reproduce on
your own hardware, and cannot leave; and when agents write and commit code
faster than people can review it, the build is the last checkpoint that has
to stay honest. Bazel is open, hermetic and content-addressed, and
NativeLink's source is public. Migrate to Bazel, then point it at
NativeLink.
...
1. **Move the build to Bazel.** Bazel's own migration guides at
   https://bazel.build/migrate cover the common starting points; the
   NativeLink team handles Bazel migrations regularly and can be reached in
   the NativeLink Slack or at ${CONTACT_EMAIL}.
...
- ${REPO}/blob/main/AGENTS.md: the repository map for an agent working on
  the code.
- ${REPO}/tree/main/.claude/skills: installable skills, including
  migrate-to-bazelmod.
...
export function renderIndex(docs: DocsCorpus): string {
  ...
  const lines: string[] = [
    ...
    `- [Skills](${REPO}/tree/main/.claude/skills): migrate-to-bazelmod and the nativelink-* playbooks.`,
