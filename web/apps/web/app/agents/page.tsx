...
const skills = [
  {
    name: "any2bazel",
    tag: "Migration",
    body: "Migrate a CMake, Maven or npm project to Bazel by iterating until Bazel's build actions match the reference build. A deterministic diff drives the loop; the agent only does the creative work.",
    credit:
      "Written at EngFlow by Ulf Adams, Armando Montañez and Yannic Staudt, released under Apache 2.0. Carried here with attribution; the edits, listed in NOTICE beside the skill, are a preface and a final step that points the migrated build at NativeLink. Thank you, Armando, Ulf and the EngFlow team.",
    href: `${REPO}/tree/main/.claude/skills/any2bazel`,
  },
  {
    ...
    <>
      ...
      <Section width="default" className="border-t border-border/60 py-24">
        ...
        <Reveal>
          <pre className="mb-8 overflow-x-auto rounded-2xl border border-border bg-surface p-5 font-mono text-[13px] leading-relaxed text-foreground/90">
            {`git clone ${REPO}\n`}
            {
              "cp -r nativelink/.claude/skills/any2bazel ~/.claude/skills/   # Claude Code, user-wide\n"
            }
            {"# then, in your agent: /any2bazel"}
          </pre>
        </Reveal>
      ...
      <section className="relative overflow-hidden border-t border-border/60">
        ...
        <Section width="narrow" className="py-28">
          <Reveal>
            <Eyebrow className="mb-5">Why we ship migration tools</Eyebrow>
            ...
            <div className="mt-6 space-y-4 text-base leading-relaxed text-muted-foreground md:text-lg">
              ...
              <p className="text-foreground">
                So: migrate to Bazel with any2bazel, modernise with migrate-to-bazelmod, and point
                the result at NativeLink. Two lines in{" "}
                <span className="font-mono text-[0.95em]">.bazelrc</span> against a local server,
                then a shared cache, then workers.
              </p>
            </div>
