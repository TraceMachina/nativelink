---
title: "Fine-tune a Language Model on CPUs using Bazel and NativeLink"
...
Most cloud infrastructure (including NativeLink) runs on x86_64 processors, while Apple Silicon Macs use ARM64. When building Bazel targets on these Macs for remote execution, architecture mismatches can occur even with explicit platform flags due to complex toolchain resolution behaviors in Bazel's ecosystem—particularly for Python targets with native dependencies.

To ensure reliable builds for x86_64 cloud environments, use either a minimal Docker container (`FROM --platform=linux/amd64 ubuntu:22.04` with just `curl` and `Bazelisk` installed) and run from your Mac's terminal using this container, or use a cloud-based Linux VM (GCP/AWS). Both approaches provide a consistent x86_64 environment where your hermetically managed dependencies will build with the correct architecture, eliminating subtle cross-compilation issues.
...
3\. Free NativeLink: the quickest run took 2 minutes of compute time while the slowest/longest took 3 minutes. Even after factoring in 1–2 additional minutes to establish a secure connection and to stream results, NativeLink Cloud provided the quickest execution times by far.
