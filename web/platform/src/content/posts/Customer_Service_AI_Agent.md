...
One of the key advantages of our setup is the ability to leverage powerful cloud resources through NativeLink's remote execution capabilities. This demo uses CPUs for fine-tuning but we've Nvidia GPUs in our infrastructure to offer; contact us to know more. Let's create a script that demonstrates how to run training jobs on remote hardware:
...
To demonstrate NativeLink’s efficacy, consistency, and reliability, we ran the same fine-tuning job on the CPU (so no GPUs) of a M1 Pro MacBook Pro, free version of Google Colab, and NativeLink (free and open-source). We executed the fine-tuning task 5 times and this is what we observed:

1\. The Mac: the quickest run took 18 minutes while the slowest/longest took 20 minutes

2\. Free version of Google Colab: the quickest run took 10 minutes while the slowest/longest took 20 minutes. The execution time was widely varied. We suspect varying traffic on Google’s servers and how Colab allocates its hardware resources played a part in this variability.
