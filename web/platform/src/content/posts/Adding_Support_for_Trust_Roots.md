---
title: "Trust Root Support in Nativelink with Neuralink"
tags: ["news", "blog-posts"]
image: https://private-user-images.githubusercontent.com/2353608/446800108-5ad5bb07-5b5d-4e10-b7b7-0528530fabbe.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..24NGJVFMUXyGuiQ2sDDWSMnPqGsDyCXf9FC713gRluo
...
Consider the previous [`TlsConfig`](https://www.nativelink.com/docs/reference/nativelink-config/#tlsconfig) structure:
...
The new implementation introduces a game-changing `use_native_roots` option in the `ClientTlsConfig`. When enabled, Nativelink automatically leverages the system's native root certificate store, eliminating the need for manual certificate file management in many common deployment scenarios.

### Technical Implementation Details

The core changes enhance Nativelink's TLS utilities with intelligent certificate handling logic:
...
2. **Traditional certificate files**: Falls back to the existing manual certificate file configuration
3. **Validation and error handling**: Ensures proper TLS configuration with clear error messages

The changes span multiple components:

- **Configuration structures**: Enhanced `ClientTlsConfig` with the new `use_native_roots` field
...
For the majority of modern deployments, you can now simply enable native roots:
...
## Customer Impact and Usability Improvements

This feature particularly benefits new Nativelink customers who previously faced significant onboarding friction. The enhanced deployment flexibility removes common blockers:

**Reduced Time-to-Value**: New customers can connect to existing infrastructure without certificate management setup, accelerating evaluation and adoption.
...
With native root certificate support, Nativelink becomes significantly more accessible to teams across diverse infrastructure environments. Whether you're running containerized workloads in Kubernetes, deploying on traditional VMs with corporate PKI, or prototyping locally, TLS configuration no longer presents a barrier to adoption.
