...
## Persistent Workers
:::note
Persistent workers are long-lived processes that handle multiple build requests without restarting.
This feature dramatically improves build performance, particularly for JVM-based languages such as Java, Kotlin, and Scala, where JVM initialization can take several seconds.
:::

### Java with Persistent Workers

First build - starts the worker JVM


```bash
bazel build //java-persistent-worker:calculator_lib \
    --config=java \
    --config=persistent-workers \
    --remote_cache=grpc://localhost:50051 \
    --remote_executor=grpc://localhost:50051
```
...
```bash
echo "// Comment added at $(date)" >> java-persistent-worker/Calculator.java
```

Incremental build - reuses warm JVM (< 200ms instead of 3-4s)
...
:::note
The first build takes normal time (~3-4s) to start the worker.
Subsequent incremental builds complete in under 200ms(thanks to the warm JVM).
This dramatic speedup makes a significant difference for iterative development.
:::
