...
Note this only applies for non-Dockerised workers, as workers in Docker can't make the new user namespace as they're
already in a [chroot](https://en.wikipedia.org/wiki/Chroot). If however you use privileged containers, this works as
per non-Docker setups.
