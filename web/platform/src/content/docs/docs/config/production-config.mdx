...
```json
{
  "stores": {
    ...
    "BEP_STORE": {
      "redis_store": {
        "addresses": ["${BEP_REDIS_STORE_URL:-redis://redis-headless:6379/2}"],
        "experimental_pub_sub_timestamp": "{{ .observed.composite.resource.spec.namespace.bep.timestamp }}"
      }
    }
  },
  "servers": [
