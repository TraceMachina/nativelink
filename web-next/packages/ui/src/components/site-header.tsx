    ...
    >
      ...
      <div className="mx-auto grid h-16 w-full max-w-[1200px] grid-cols-[1fr_auto_1fr] items-center gap-4 px-6">
        ...
        <nav aria-label="Primary" className="hidden md:block">
          <ul className="flex items-center gap-1 rounded-full border border-border/70 bg-surface/60 px-1.5 backdrop-blur">
            {links.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
        ...
        <div className="flex items-center justify-self-end gap-1.5">
          <a
            ...
            className="hidden h-10 w-10 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-foreground/5 hover:text-foreground sm:inline-flex"
          >
