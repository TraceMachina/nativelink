{
  ...
  #   shellHook = ''
  #   # Generate the `.bazelrc.lre` config file.
  ...
  configFile = pkgs.runCommand ".bazelrc.ll" {} ''
  ...
  options = {
    installationScript = lib.mkOption {
      ...
      description = lib.mkDoc ''
        A bash snippet which creates a .bazelrc.lre file in the repository.
      '';
    };
    Env = lib.mkOption {
      ...
      description = lib.mdDoc ''
        ...
        The evaluated contents of `Env` are printed in `.bazelrc.lre`, this
    ...
    prefix = lib.mkOption {
      type = lib.types.str;
      description = lib.mdDoc ''
        An optional Bazel config prefix for the flags in `.bazelrc.lre`.
  ...
  config = {
    installationScript = ''
      if ! type -t git >/dev/null; then
        ...
        if ! readlink "''${GIT_WC}/.bazelrc.lre" >/dev/null \
          || [[ $(readlink "''${GIT_WC}/.bazelrc.lre") != ${configFile} ]]; then
          echo 1>&2 "LRE: updating $PWD repository"
          [ -L .bazelrc.lre ] && unlink .bazelrc.lre

          if [ -e "''${GIT_WC}/.bazelrc.lre" ]; then
            echo 1>&2 "LRE: WARNING: Refusing to install because of pre-existing .bazelrc.lre"
            echo 1>&2 "  Remove the .bazelrc.lre file and add .bazelrc.lre to .gitignore."
          else
            ln -fs ${configFile} "''${GIT_WC}/.bazelrc.lre"
          fi
        fi
      fi
    '';
