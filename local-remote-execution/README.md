...
`nix develop`) it will generate a file called `.bazelrc.lre` which contains
...
an optional `prefix` input to configure the generated `.bazelrc.lre`:
...
The commented paths created in `.bazelrc.lre` technically have no effect on what
...
printed in `.bazelrc.lre` will be available on your local system.

Let's move on to Bazel's configuration.

## 🌱 Bazel-side setup

First, hook the generated `.bazelrc.lre` into the main `.bazelrc` with a
`try-import`:
...
you should find the same `/nix/store/...` paths as in the `.bazelrc.lre` file.
