# FIXME(palfrey): Copy of https://github.com/NixOS/nixpkgs/blob/nixos-unstable/pkgs/by-name/re/renovate/package.nix#L120 because
# macOS builds bug out with a lack of libtool, so we need to fix that
{
  lib,
  stdenv,
  fetchFromGitHub,
...
in
  stdenv.mkDerivation (finalAttrs: {
    ...
    nativeBuildInputs =
      ...
      ++ lib.optional stdenv.hostPlatform.isDarwin [xcbuild cctools];
