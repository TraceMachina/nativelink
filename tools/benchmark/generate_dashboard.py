...
def parse_args():
    ...
    parser.add_argument("--projects", default="rustlings,googletest", help="Comma-separated list of projects")
    parser.add_argument("--metrics", default="build_time,network_usage", help="Comma-separated list of metrics")
    ...
    for project in projects:
        ...
        for test_name in data[project][0].get('metrics', {}):
            for metric in metrics:
                if not any(metric in result.get('metrics', {}).get(test_name, {}) 
                ...
                for result in data[project]:
                    if (test_name in result.get('metrics', {}) and 
                ...
                for i, (date, value, commit) in enumerate(zip(dates, values, commits)):
                    plt.annotate(commit, (date, value), 
...
def generate_html(data, output_dir, projects, metrics):
    ...
    for project in projects:
        ...
        for test_name in data[project][0].get('metrics', {}):
            for metric in metrics:
                if any(metric in result.get('metrics', {}).get(test_name, {}) 
                      for result in data[project]):
        ...
        for result in data[project][-10:]:
            ...
            for test_name in data[project][0].get('metrics', {}):
                for metric in metrics:
                    if any(metric in r.get('metrics', {}).get(test_name, {}) 
