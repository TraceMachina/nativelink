...
def create_interactive_time_series(results: List[Dict], 
                                  metric: str = "build_time",
                                  test_names: Optional[List[str]] = None,
                                  last_n: int = 10,
                                  title_prefix: str = "NativeLink Performance",
                                  annotations: Optional[Dict] = None) -> go.Figure:
    ...
    if not data:
        ...
        fig = go.Figure()
        fig.add_annotation(text=f"No data found for metric: {metric}",
...
def create_interactive_comparison_chart(results: List[Dict],
                                       metric: str = "build_time",
                                       test_names: Optional[List[str]] = None,
                                       last_n: int = 2,
                                       specific_commits: Optional[List[str]] = None) -> go.Figure:
    ...
    if len(recent_results) < 2:
        ...
        fig = go.Figure()
        fig.add_annotation(text="Need at least 2 results for comparison",
                          xref="paper", yref="paper",
                          x=0.5, y=0.5, showarrow=False)
        return fig
...
def create_interactive_heatmap(results: List[Dict],
                              test_names: Optional[List[str]] = None,
                              metrics: Optional[List[str]] = None,
                              last_n: int = 5) -> go.Figure:
    ...
    if len(recent_results) < 2:
        ...
        fig = go.Figure()
        fig.add_annotation(text="Need at least 2 results for heatmap",
                          xref="paper", yref="paper",
                          x=0.5, y=0.5, showarrow=False)
        return fig
    ...
    if not data:
        ...
        fig = go.Figure()
        fig.add_annotation(text="No data found for heatmap",
                          xref="paper", yref="paper",
                          x=0.5, y=0.5, showarrow=False)
        return fig
    ...
    melted_df = pd.melt(pct_change_df, 
                        id_vars=["test", "metric"], 
...
def generate_interactive_html_report(results: List[Dict],
                                    output_dir: str,
    ...
    for test_name, metrics in metrics_by_test.items():
        for metric in metrics:
            ...
            chart_div += f"<div class='chart-title'>{test_name} - {metric.replace('_', ' ').title()}</div>"
        ...
        for test_name, metrics in latest.get("metrics", {}).items():
            for metric, value in metrics.items():
                if test_name in previous.get("metrics", {}) and metric in previous["metrics"][test_name]:
                    ...
                    html_content.append(f"            <td class='{change_class}'>{change:.2f}%</td>")
                    html_content.append(f"        </tr>")
    ...
    html_content.append("            <span class='nav-link' onclick='document.getElementById(\"performance_summary\").scrollIntoView({behavior: \"smooth\"})'>Summary</span>")
    html_content.append("            <span class='nav-link' onclick='document.getElementById(\"performance_charts\").scrollIntoView({behavior: \"smooth\"})'>Charts</span>")
    html_content.append("            <span class='nav-link' onclick='document.getElementById(\"performance_heatmap\").scrollIntoView({behavior: \"smooth\"})'>Heatmap</span>")
...
def main():
    parser = argparse.ArgumentParser(description="Interactive NativeLink Benchmark Visualization Tool")
    ...
    parser.add_argument("--annotations", 
                        help="Path to JSON file with annotations for significant changes")
    parser.add_argument("--create-sample-annotations", action="store_true",
                        help="Create a sample annotations file")
    ...
    time_series_path = os.path.join(
        args.output_dir, 
        f"{project_prefix}time_series_{args.metric}.html"
    )
    ...
    comparison_path = os.path.join(
        args.output_dir, 
        f"{project_prefix}comparison_{args.metric}.html"
    )
    ...
    heatmap_path = os.path.join(
        args.output_dir, 
        f"{project_prefix}performance_heatmap.html"
    )
