...
def create_time_series_chart(
    ...
    if not data:
        ...
        ax.text(
            0.5, 0.5, f"No data found for metric: {metric}", ha="center", va="center", fontsize=12
        )
        return fig
...
def create_comparison_chart(
    ...
    if len(pivot_df.columns) == 2:
        ...
        for i, test in enumerate(pivot_df.index):
            ...
            ax.annotate(
                f"{pct_change:.1f}%", xy=(i, y_pos), ha="center", color=color, fontweight="bold"
            )
...
def create_heatmap(
    ...
    if len(recent_results) < 2:
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.text(
            0.5, 0.5, "Need at least 2 results for heatmap", ha="center", va="center", fontsize=12
        )
        return fig
    ...
    if not data:
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.text(0.5, 0.5, "No data found for heatmap", ha="center", va="center", fontsize=12)
        return fig
    ...
    cmap = LinearSegmentedColormap.from_list(
        "green_white_red", [(0, "green"), (0.5, "white"), (1, "red")]
    )
        ...
        for test_name, metrics in latest.get("metrics", {}).items():
            for metric, value in metrics.items():
                if (
                    ...
                    html_content.append(
                        f"            <td style='color: {change_color}'>{change:.2f}%</td>"
                    )
                    html_content.append(f"        </tr>")
...
def main():
    ...
    time_series_path = os.path.join(
        args.output_dir, f"{project_prefix}time_series_{args.metric}.png"
    )
    ...
    comparison_path = os.path.join(args.output_dir, f"{project_prefix}comparison_{args.metric}.png")
    ...
    # Save heatmap
    heatmap_path = os.path.join(args.output_dir, f"{project_prefix}performance_heatmap.png")
