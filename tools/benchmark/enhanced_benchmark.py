...
def clean_bazel_workspace(project_dir: str):
    ...
    try:
        # Check if cleaning is actually needed by looking for bazel-* directories
        bazel_dirs = list(Path(project_dir).glob("bazel-*"))
        if not bazel_dirs:
            print("  No Bazel output directories found, skipping clean")
            return
            
        # Run clean with timeout
        run_command(["bazel", "clean", "--expunge"], cwd=project_dir)
    except subprocess.TimeoutExpired:
        print("  Warning: Bazel clean timed out")
    except Exception as e:
        print(f"  Error cleaning Bazel workspace: {e}")
...
def main():
    ...
    parser.add_argument(
        "--incremental", action="store_true", default=True, help="Enable incremental build tests"
    )
    parser.add_argument(
        "--network-stats",
        action="store_true",
        default=True,
        help="Enable network statistics collection",
    )
    ...
    # Check for Bazel workspace and validate project structure
    if not any(
    ...
    # Validate project has buildable targets
    try:
        ...
        if not targets:
            print(f"Error: No buildable targets found in project directory '{project_dir}'")
            sys.exit(1)
    except subprocess.CalledProcessError as e:
        print(f"Error validating Bazel project: {e.stderr.decode().strip()}")
        sys.exit(1)
    ...
    benchmarks = [
        {
            ...
            "args": [
                ...
                "--remote_upload_local_results=false",  # Disable upload to cache
            ],
        },
        {
        ...
        if make_incremental_change(project_dir):
            ...
            if not os.path.exists(test_data_dir):
                ...
                with open(os.path.join(test_data_dir, "sample.txt"), "w") as f:
                    f.write("Test data generated by benchmark script\n")
