...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate visualizations for NativeLink benchmarks"
    )
    ...
    parser.add_argument(
        "--days", type=int, default=30, help="Number of days to include in trend charts"
    )
...
def load_benchmark_data(input_dir, project=None, days=30):
    ...
    for project_name in data:
        ...
        if days > 0:
            ...
            data[project_name] = [
                r
                for r in data[project_name]
                if datetime.strptime(
                    r.get("timestamp", "19700101_000000"), "%Y%m%d_%H%M%S"
                ).timestamp()
                > cutoff
            ]
    ...
    for project_name, project_data in data.items():
        ...
        for benchmark_type in project_data[0].get("metrics", {}).keys():
            ...
            for metric in metrics:
                ...
                for result in project_data:
                    if (
                        benchmark_type in result.get("metrics", {})
                        and metric in result["metrics"][benchmark_type]
                    ):
...
"""
    ...
    for project_name, project_data in data.items():
        ...
        html_content = f"""
            ...
            """  # Start of multi-line string  # Added closing parenthesis
