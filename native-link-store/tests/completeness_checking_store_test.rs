...
mod completeness_checking_store_tests {
    ...
    const HASH4: &str = "0124456789abcdef000000000000000000000000000000000123456789abcdef";
    const HASH5: &str = "0121f56789abcdef000000000000000000000000000000000123456789abcdef";
    const HASH6: &str = "0129456789abcdef000000000000000000000000000000000123456789abcdef";
    const HASH7: &str = "0128456789abcdef000000000000000000000000000000000123456789abcdef";
    ...
    #[tokio::test]
    async fn verify_completeness_check_none_missing() -> Result<(), Error> {
        ...
        let file4_digest = DigestInfo::try_new(HASH4, FILE3_CONTENT.len())?;
        let file5_digest = DigestInfo::try_new(HASH5, FILE3_CONTENT.len())?;
        let output_file_digest = DigestInfo::try_new(HASH6, FILE3_CONTENT.len())?;
        ...
        pinned_cas.update_oneshot(file5_digest, FILE3_CONTENT.into()).await?;
        pinned_cas
        ...
        pinned_cas
            .update_oneshot(tree_digest, tree_bytes.clone().into())
            .await?;
        ...
        let action_result = ProtoActionResult {
            ...
            stdout_digest: Some(file5_digest.into()),
            stderr_raw: Default::default(), // We don't implement this.
            stderr_digest: Some(file5_digest.into()),
        ...
        pinned_cas.update_oneshot(digest, bytes.into()).await?;

        let res = pinned_store.has_many(&digests).await;
        trace!("RES_OK: {:?}", res);
        assert!(res.is_ok(), "Expected has_many to succeed with items in cas.");
        // Res should contain one None
        assert!(res.unwrap()[0].is_some(), "All items should show as existing in CAS.");

        Ok(())
    }

    #[tokio::test]
    async fn verify_completeness_check_output_file_missing_in_cas() -> Result<(), Error> {
        let ac_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let cas_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let store_owned = CompletenessCheckingStore::new(ac_store.clone(), cas_store.clone());
        let pinned_store: Pin<&dyn Store> = Pin::new(&store_owned);
        ...
        assert!(
            res_clone.unwrap()[0].is_none(),
            "Second digest should show up as none as file digests are missing in CAS."
        );
    ...
    #[tokio::test]
    async fn verify_completeness_check_root_file_missing() -> Result<(), Error> {
        let ac_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let cas_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let store_owned = CompletenessCheckingStore::new(ac_store.clone(), cas_store.clone());
        let pinned_store: Pin<&dyn Store> = Pin::new(&store_owned);
        ...
        assert!(
            res_clone.unwrap()[0].is_none(),
            "Second digest should show up as none as root file is missing in CAS."
        );
    ...
    #[tokio::test]
    async fn verify_completeness_check_child_file_missing() -> Result<(), Error> {
        let ac_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let cas_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let store_owned = CompletenessCheckingStore::new(ac_store.clone(), cas_store.clone());
        let pinned_store: Pin<&dyn Store> = Pin::new(&store_owned);
        ...
        assert!(
            res_clone.unwrap()[0].is_none(),
            "Second digest should show up as none as child file digests are missing in CAS."
        );
    ...
    #[tokio::test]
    async fn verify_completeness_check_stdout_and_stderr_missing() -> Result<(), Error> {
        let ac_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let cas_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let store_owned = CompletenessCheckingStore::new(ac_store.clone(), cas_store.clone());
        let pinned_store: Pin<&dyn Store> = Pin::new(&store_owned);
        ...
        let file4_digest = DigestInfo::try_new(HASH4, FILE3_CONTENT.len())?;
        let file5_digest = DigestInfo::try_new(HASH5, FILE3_CONTENT.len())?;
        let file6_digest = DigestInfo::try_new(HASH6, FILE1_CONTENT.len())?;
        let output_file_digest = DigestInfo::try_new(HASH7, FILE3_CONTENT.len())?;
        ...
        pinned_cas
            .update_oneshot(tree_alt_digest, tree_alt_bytes.clone().into())
            .await?;
        ...
        let od_clone = output_directory.clone();
        let od1_clone = output_directory_1.clone();
        ...
        let action_result = ProtoActionResult {
            ...
            stdout_digest: Some(file5_digest.into()),
            stderr_raw: Default::default(), // We don't implement this.
            stderr_digest: Some(file6_digest.into()),
            execution_metadata: Some(ExecutedActionMetadata {
                worker: "".to_string(),
                queued_timestamp: Some(make_system_time(1).into()),
                worker_start_timestamp: Some(make_system_time(2).into()),
        ...
        let action_result_1 = ProtoActionResult {
            output_files: vec![OutputFile {
                path: "some path1".to_string(),
                digest: Some(output_file_digest.into()),
                is_executable: true,
            ...
            output_file_symlinks: Default::default(),
            output_symlinks: Default::default(),
            output_directories: vec![od_clone, od1_clone],
            output_directory_symlinks: Default::default(), // Bazel deprecated this.
            exit_code: 5,
            stdout_raw: Default::default(), // We don't implement this.
            stdout_digest: Some(file6_digest.into()),
            stderr_raw: Default::default(), // We don't implement this.
            stderr_digest: Some(file5_digest.into()),
            execution_metadata: Some(ExecutedActionMetadata {
                worker: "".to_string(),
        ...
        let mut bytes = vec![];
        action_result_1.encode(&mut bytes)?;

        let dummy_hash = [0u8; 32];

        let digest_2 = DigestInfo::new(dummy_hash, bytes.len() as i64);

        let digests = vec![digest, digest_2];
        ...
        assert!(res.is_ok(), "Expected has_many to succeed with items in cas.");
        let res_clone = res.clone();
        assert!(res.unwrap()[0].is_none() && res_clone.unwrap()[1].is_none(), "Both digests should show up as none as stderr and stdout is missing in CAS for the first and second ActionResult respectively.");

        Ok(())
    }

    #[tokio::test]
    async fn verify_completeness_check_one_some_one_none() -> Result<(), Error> {
        let ac_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let cas_store = Arc::new(MemoryStore::new(&MemoryStoreConfig::default()));
        let store_owned = CompletenessCheckingStore::new(ac_store.clone(), cas_store.clone());
        let pinned_store: Pin<&dyn Store> = Pin::new(&store_owned);
        ...
        let digests = vec![digest, digest_2, digest];

        pinned_cas.update_oneshot(digest_2, bytes.into()).await?;

        let res = pinned_store.has_many(&digests).await;
        trace!("RES_OK: {:?}", res);
        assert!(res.is_ok(), "Expected has_many to succeed with items in cas.");
        let res_clone = res.clone();
        let res_clone_1 = res.clone();
        assert!(
            res.unwrap()[0].is_none() && res_clone.unwrap()[1].is_some() && res_clone_1.unwrap()[2].is_some(),
            "Second digest should show up as Some."
        );
