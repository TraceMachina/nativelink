...
mod ref_store_tests {
    ...
    use native_link_store::size_partitioning_store::SizePartitioningStore;
    ...
    fn setup_stores(size: u64) -> (SizePartitioningStore, Arc<MemoryStore>, Arc<MemoryStore>) {
        let lower_memory_store = Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default()));
        let upper_memory_store = Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default()));

        let size_part_store = SizePartitioningStore::new(
            &native_link_config::stores::SizePartitioningStore {
                size,
                lower_store: native_link_config::stores::StoreConfig::memory(
                    native_link_config::stores::MemoryStore::default(),
                ),
                upper_store: native_link_config::stores::StoreConfig::memory(
                    native_link_config::stores::MemoryStore::default(),
                ),
            },
