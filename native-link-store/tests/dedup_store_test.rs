...
use native_link_store::dedup_store::DedupStore;
...
fn make_default_config() -> native_link_config::stores::DedupStore {
    native_link_config::stores::DedupStore {
        index_store: native_link_config::stores::StoreConfig::memory(native_link_config::stores::MemoryStore::default()),
        content_store: native_link_config::stores::StoreConfig::memory(
...
mod dedup_store_tests {
    ...
    async fn simple_round_trip_test() -> Result<(), Error> {
        let store_owned = DedupStore::new(
            &make_default_config(),
            Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default())), // Index store.
            Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default())), // Content store.
        );
    ...
    async fn check_missing_last_chunk_test() -> Result<(), Error> {
        let content_store = Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default()));
        let store_owned = DedupStore::new(
            &make_default_config(),
            Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default())), // Index store.
            content_store.clone(),
    ...
    async fn check_length_not_set_with_chunk_read_beyond_first_chunk_regression_test() -> Result<(), Error> {
        let store_owned = DedupStore::new(
            &native_link_config::stores::DedupStore {
                index_store: native_link_config::stores::StoreConfig::memory(
                    native_link_config::stores::MemoryStore::default(),
                ),
                content_store: native_link_config::stores::StoreConfig::memory(
                    native_link_config::stores::MemoryStore::default(),
    ...
    async fn has_checks_content_store() -> Result<(), Error> {
        let index_store = Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore::default()));
        let content_store = Arc::new(MemoryStore::new(&native_link_config::stores::MemoryStore {
