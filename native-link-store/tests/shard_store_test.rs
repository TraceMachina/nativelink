...
use native_link_store::shard_store::ShardStore;
...
fn make_stores(weights: &[u32]) -> (Arc<ShardStore>, Vec<Arc<dyn Store>>) {
    let memory_store_config = native_link_config::stores::MemoryStore::default();
    let store_config = native_link_config::stores::StoreConfig::memory(memory_store_config.clone());
    ...
    let shard_store = Arc::new(
        ShardStore::new(
            &native_link_config::stores::ShardStore {
                stores: weights
                    .iter()
                    .map(|weight| native_link_config::stores::ShardConfig {
