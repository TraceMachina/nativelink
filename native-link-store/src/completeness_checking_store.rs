...
impl Store for CompletenessCheckingStore {
    async fn has_with_results(
        ...
        let mut digest_vec = Vec::new();
        let has_request_outstanding = Arc::new(AtomicBool::new(false));
        let none_found: Arc<Mutex<HashSet<usize>>> = Arc::new(Mutex::new(HashSet::new()));
        let mut last_len = 0;
        ...
        while let Some(future_result) = futures.next().await {
            match future_result {
                ...
                FutureResult::AddFuturesAndDigests((futures_to_add, digest_infos)) => {
                    ...
                    digest_vec.extend(digest_infos);
                    if !digest_vec.is_empty() && !has_request_outstanding.load(Ordering::Acquire) {
                        let has_request_outstanding = has_request_outstanding.clone();
                        let none_found = none_found.clone();
                        has_request_outstanding.store(true, Ordering::Release);

                        let (digests, indexes): (Vec<_>, Vec<_>) = futures::stream::iter(digest_vec.drain(..))
                            .filter(|(_digest, index)| {
                                let none_found = none_found.clone();
                                let index = *index;
                                ...
                                async move {
                                    let contains;
                                    {
                                        let none_found = none_found.lock().await;
                                        contains = none_found.contains(&index);
                                    }
                                    !contains
                                }
                            })
                        ...
                        for i in 0..last_len {
                            let none_found = none_found.lock().await;
                            if none_found.contains(&last_indexes[i]) {
                                results[last_indexes[i]] = None;
                                break;
                            } else {
                                results[last_indexes[i]] = Some(last_len);
                            }
                        }

                        if !digests.is_empty() {
                            last_len = digests.len();
                            last_indexes = indexes.clone();
                        }
                        ...
                        futures.push(
                            async move {
                                ...
                                let res_list = self.pin_cas().has_many(&digests).await.unwrap();

                                for (i, result) in res_list.iter().enumerate() {
                                    if result.is_none() {
                                        let mut none_found = none_found.lock().await;
