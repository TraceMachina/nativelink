...
pub struct GrpcStore {
    ...
    store_type: native_link_config::stores::StoreType,
...
impl GrpcStore {
    pub async fn new(config: &native_link_config::stores::GrpcStore) -> Result<Self, Error> {
    ...
    pub async fn new_with_jitter(
        config: &native_link_config::stores::GrpcStore,
        jitter_fn: Box<dyn Fn(Duration) -> Duration + Send + Sync>,
    ) -> Result<Self, Error> {
