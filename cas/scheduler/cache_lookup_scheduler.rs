...
impl ActionScheduler for CacheLookupScheduler {
    ...
    async fn add_action(
        ...
        tokio::spawn(async move {
            ...
            let rx = action_scheduler.add_action(name, action_info).await.map_or_else(
                |err| {
                    Arc::make_mut(&mut current_state).stage = ActionStage::Error((err, ActionResult::default()));
                    watch::channel(current_state).1
                },
                |rx| rx,
            );
            WatchStream::new(rx)
                .for_each(|action_state| async {
                    let _ = tx.send(action_state);
                })
                .await;
        });
        Ok(rx)
