...
impl CacheLookupScheduler {
    ...
    async fn validate_outputs_exist(&self, action_result: &ProtoActionResult, instance_name: String) -> bool {
        ...
        if let Some(grpc_store) = maybe_grpc_store {
            ...
            let mut check_futures: FuturesUnordered<_> = required_digests
                .iter()
                .map(|digest| async move { cas_pin.has(DigestInfo::try_from(digest.clone())?).await })
                .collect();
            while let Some(result) = check_futures.next().await {
                if result.is_err() || result.is_ok_and(|result| result.is_none()) {
                    return false;
                }
            }
            true
...
impl ActionScheduler for CacheLookupScheduler {
    ...
    async fn add_action(&self, action_info: ActionInfo) -> Result<watch::Receiver<Arc<ActionState>>, Error> {
        if !action_info.skip_cache_lookup {
            ...
            if let Some(proto_action_result) = self.get_action_from_store(action_digest, instance_name.clone()).await {
                if self.validate_outputs_exist(&proto_action_result, instance_name).await {
        ...
        self.action_scheduler.add_action(action_info).await
    }
}
