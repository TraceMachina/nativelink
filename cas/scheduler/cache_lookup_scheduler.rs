    ...
    async fn add_action(
        ...
        tokio::spawn(async move {
            ...
            if let Some(proto_action_result) =
                get_action_from_store(ac_store, &action_digest, instance_name.clone()).await
            {
            ...
            match action_scheduler.add_action(name, action_info).await {
                Ok(rx) => {
                    let mut rx_stream = WatchStream::new(rx);
                    while let Some(action_state) = rx_stream.next().await {
                ...
                Err(err) => {
                    log::error!(
                        "\x1b[0;31madd_action unable to forward to upstream scheduler\x1b[0m: {}",
                        err
                    );
