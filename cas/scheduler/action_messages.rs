...
impl From<FileInfo> for OutputFile {
    fn from(val: FileInfo) -> Self {
        ...
        Self {
            ...
            digest: Some((&val.digest).into()),
            is_executable: val.is_executable,
            contents: tonic::codegen::Bytes::default(),
            node_properties: Option::default(), // Not supported.
        }
    }
}
...
impl From<ActionStage> for ExecuteResponse {
    fn from(val: ActionStage) -> Self {
        ...
        let logs_from = |result: &ActionResult| -> HashMap<String, LogFile> {
            let mut logs = HashMap::with_capacity(result.server_logs.len());
            for (k, v) in &result.server_logs {
                logs.insert(
                    k.clone(),
                    LogFile {
                        digest: Some(v.into()),
        ...
        match val {
            ...
            ActionStage::Completed(action_result) => {
                let server_logs = logs_from(&action_result);
                Self {
            ...
            ActionStage::Error((error, action_result)) => {
                let server_logs = logs_from(&action_result);
                Self {
                    result: Some(action_result.into()),
                    cached_result: false,
                    status: Some(error.into()), // map_or(Status::default(), |v| v.into())),
                    server_logs,
                    message: RESPONSE_MESSAGE.to_string(),
                }
            }
...
impl From<ActionResult> for ProtoActionResult {
    fn from(val: ActionResult) -> Self {
        ...
        Self {
            ...
            stdout_raw: tonic::codegen::Bytes::default(),
            stdout_digest: Some(val.stdout_digest.into()),
            stderr_raw: tonic::codegen::Bytes::default(),
