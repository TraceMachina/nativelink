...
use config::schedulers::{PlatformPropertyAddition, PropertyModification};
use error::Error;
...
mod property_modifier_scheduler_tests {
    ...
    async fn add_action_overwrites_property() -> Result<(), Error> {
        ...
        assert_eq!(
            HashMap::from([(name, PlatformPropertyValue::Unknown(replaced_value))]),
            action_info.platform_properties.properties
        );
        Ok(())
    }
