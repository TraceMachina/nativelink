...
mod cache_lookup_scheduler_tests {
    ...
    async fn add_action_does_cache_lookup() -> Result<(), Error> {
        ...
        store_pin
            ...
            .await?;
        let watch_channel = cache_scheduler
            .add_action("name".to_string(), action_info.clone())
            .await?;
        let mut watch_stream = WatchStream::new(watch_channel);
        if watch_stream.next().await.err_tip(|| "")?.stage != ActionStage::CacheCheck {
            panic!("Not performing a cache check");
        }
        let cached_action_state = watch_stream.next().await.err_tip(|| "")?;
    ...
    async fn add_action_validates_outputs() -> Result<(), Error> {
        ...
        let _ = join!(
            cache_scheduler.add_action("name".to_string(), action_info),
    ...
    async fn add_action_handles_skip_cache() -> Result<(), Error> {
        ...
        let _ = join!(
            cache_scheduler.add_action("name".to_string(), skip_cache_action),
            mock_scheduler.expect_add_action(Ok(forward_watch_channel_rx))
        );
        Ok(())
    }
