...
async fn test_walk_ac_and_order_items() -> Result<(), Error> {
    ...
    const FILE3_NAME: &str = "file3.exec";
    const FILE3_CONTENT: &str = "HELLOFILE3";

    let ac_store = Arc::new(MemoryStore::new(&config::stores::MemoryStore::default()));
    let pinned_store: Pin<&dyn Store> = Pin::new(ac_store.as_ref());

    let file1_digest = DigestInfo::try_new(FILE1_NAME, FILE1_CONTENT.len())?;
    let file2_digest = DigestInfo::try_new(FILE2_NAME, FILE2_CONTENT.len())?;
    let file3_digest = DigestInfo::try_new(FILE3_NAME, FILE3_CONTENT.len())?;

    let timestamp1 = UNIX_EPOCH + Duration::from_secs(100);
    let timestamp2 = UNIX_EPOCH + Duration::from_secs(200);
    let timestamp3 = UNIX_EPOCH + Duration::from_secs(300);
    ...
    let action_result1 = ActionResult {
        ...
        exit_code: 0,
        stdout_digest: Default::default(),
        stderr_digest: Default::default(),
    ...
    let sorted_items = walk_ac_and_order_items(digests, &store).await?;

    assert!(sorted_items == vec![file3_digest, file2_digest, file1_digest]);
