...
pub struct ShardStore {
    ...
    weights_and_stores: Vec<(u32, Arc<dyn StoreTrait>)>,
}
...
impl ShardStore {
    ...
    fn get_store(&self, digest: &DigestInfo) -> Pin<&dyn StoreTrait> {
...
impl StoreTrait for ShardStore {
    async fn has_with_results(
        ...
        if digests.len() == 1 {
            ...
            let store = self.get_store(&digests[0]);
            store.has_with_results(digests, results).await?;
            return Ok(());
        }
        let mut digests_for_store: Vec<_> = self
            .weights_and_stores
            .iter()
        ...
        while let Some((digest_idxs, inner_results)) = future_stream.try_next().await? {
            for (digest_idx, inner_result) in digest_idxs.into_iter().zip(inner_results.into_iter()) {
