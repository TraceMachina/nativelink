...
use aws_sdk_s3::{
    ...
    types::{
        builders::{CompletedMultipartUploadBuilder, CompletedPartBuilder},
        CompletedPart,
    },
    Client,
};
        ...
        loop {
            match self.connector.call(req.clone()).await {
                ...
                Err(_) if retries < max_retries => {
                    log::warn!("S3 connection temporarily dropped");
                    retries += 1;
                    sleep(delay).await;
                }
                Err(e) => {
                    return Err(make_err!(
                        Code::Unavailable,
                        "Future failed after {max_retries} retries: {e}"
                    ))
                }
            }
        }
...
impl Service<Uri> for TlsConnector {
    ...
    fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.connector
            .poll_ready(cx)
            .map_err(|e| make_err!(Code::Unavailable, "Failed poll: {}", e))
    }
    ...
    fn call(&mut self, req: Uri) -> Self::Future {
        ...
        Box::pin(async move { connector_clone.simple_retry(req, MAX_RETRIES, RETRY_DELAY).await })
    }
}

fn should_retry<T, E>(result: Result<T, SdkError<E>>) -> RetryResult<T>
where
    SdkError<E>: std::fmt::Display,
{
    match result {
        ...
        Err(SdkError::DispatchFailure(_)) => {
            RetryResult::Retry(make_err!(Code::Unavailable, "Dispatch failure. Retrying..."))
        }

        // TODO(aaronmondal): Consider explicit handling of these `SDKError`s:
...
impl S3Store {
    pub async fn new(config: &config::stores::S3Store) -> Result<Self, Error> {
        let s3_client = {
            let http_client = HyperClientBuilder::new().build(TlsConnector::new());
    ...
    async fn update(
        ...
        let complete_result = {
            ...
            loop {
                ...
                let inner_bucket = self.bucket.clone();
            ...
            let completed_parts = CompletedMultipartUploadBuilder::default()
                .set_parts(Some(try_join_all(completed_part_futs).await?))
                .build();
        ...
        self.retrier
            .retry(
                ...
                unfold(writer, move |writer| async move {
                    ...
                    if result.is_err() {
                        return Some((
                            // TODO(aaronmondal): Try to print a more
                            // informative message in an elegant way that
                    ...
                    let mut s3_in_stream = match should_retry(result) {
                        RetryResult::Ok(get_object_output) => get_object_output.body,
                        RetryResult::Err(err) => {
                    ...
                    while let Some(maybe_bytes) = s3_in_stream.next().await {
                        match maybe_bytes {
                            ...
                            Err(e) => {
                                ...
                                return Some((
                                    RetryResult::Err(make_input_err!("Bad bytestream element: {e}")),
                                    writer,
                                ));
                            }
                        }
                    ...
                    if let Err(e) = writer.send_eof().await {
                        return Some((
                            RetryResult::Err(make_input_err!("Failed to send EOF to consumer: {e}")),
                            writer,
                        ));
                    }
