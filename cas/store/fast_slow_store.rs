...
impl StoreTrait for FastSlowStore {
    ...
    fn register_metrics(self: Arc<Self>, registry: &mut Registry) {
        let mut fast_store_registry = registry.sub_registry_with_prefix("fast");
        self.fast_store.clone().register_metrics(&mut fast_store_registry);
        let mut slow_store_registry = registry.sub_registry_with_prefix("slow");
        self.slow_store.clone().register_metrics(&mut slow_store_registry);
    }
