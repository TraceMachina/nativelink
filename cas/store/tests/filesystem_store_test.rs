...
mod filesystem_store_tests {
    ...
    async fn file_continues_to_stream_on_content_replace_test() -> Result<(), Error> {
        ...
        let spawn_fut = tokio::spawn(async move { Pin::new(store_clone.as_ref()).get(digest1_clone, writer).await });
    ...
    async fn file_gets_cleans_up_on_cache_eviction() -> Result<(), Error> {
        ...
        let mut reader = {
            let (writer, mut reader) = make_buf_channel_pair();
            let store_clone = store.clone();
            let spawn_fut = tokio::spawn(async move { Pin::new(store_clone.as_ref()).get(digest1, writer).await });
            reader
        };
