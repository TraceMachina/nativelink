...
impl FileEntry {
    ...
    pub async fn get_file_path_locked<T, Fut: Future<Output = Result<T, Error>>, F: FnOnce(String) -> Fut>(
        ...
        handler(to_full_path_from_digest(
            encoded_file_path.path_type.extract_path(&self.folder_paths),
            &encoded_file_path.digest,
        ))
        .await
    }
}
...
impl LenEntry for FileEntry {
    ...
    async fn unref(&self) {
        ...
        }
        {
            ...
            let from_path = to_full_path_from_digest(
                encoded_file_path.path_type.extract_path(&self.folder_paths),
                &encoded_file_path.digest,
            );
...
impl Drop for FileEntry {
    fn drop(&mut self) {
        ...
        let file_path = to_full_path_from_digest(
            encoded_file_path.path_type.extract_path(&self.folder_paths),
            &encoded_file_path.digest,
        );
...
impl FilesystemStore {
    ...
    async fn update_file<'a>(
        ...
        //    contents until we relese the lock.
        {
            ...
            let final_path = to_full_path_from_digest(
                encoded_file_path.path_type.extract_path(&self.folder_paths),
                &encoded_file_path.digest,
            );
