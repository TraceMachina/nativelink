...
impl FilesystemStore {
    ...
    async fn update_file<'a>(
        ...
        //    item is not possible within the `insert()` call because the mutex inside the eviction
        //    map. If an eviction of new item happens after `insert()` but before `rename()` then
        //    we get to finish our operation because the `unref()` of the new item will be blocked
        //    on us because we currently have the mutex locked.
