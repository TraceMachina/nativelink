...
impl StoreTrait for GrpcStore {
    ...
    async fn has_with_results(
        ...
        // Need to match the missing blobs against the pre-populated results.
        // Since the ordering is not guaranteed above, the matching has to check
        // every entry against every other entry which is not very efficient.
        // Therefore, two structures are created that are sorted on the
        // DigestInfo which means that matching will always occur in order.
        let mut digests_and_results = digests.iter().zip(results.iter_mut()).collect::<BinaryHeap<_>>();
        let mut ordered_missing_digests = BinaryHeap::with_capacity(missing_blobs_response.missing_blob_digests.len());
        for missing_digest in missing_blobs_response.missing_blob_digests {
            ordered_missing_digests.push(DigestInfo::try_from(missing_digest)?);
        }
        // Now the ordered structures are created we can iterate over them in
        // order to perform the matching.
        while let Some(missing_digest) = ordered_missing_digests.pop() {
            let mut found = false;
            while let Some((digest, result)) = digests_and_results.pop() {
                if &missing_digest == digest {
                    *result = None;
                    found = true;
                    break;
                }
            ...
            if !found {
                return Err(make_input_err!("Missing digest not in request list"));
            }
        }
