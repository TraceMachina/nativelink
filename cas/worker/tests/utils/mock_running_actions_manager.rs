...
enum RunningActionManagerCalls {
    ...
    SaveActionInStore((DigestInfo, ActionResult)),
}
...
impl MockRunningActionsManager {
    ...
    pub async fn expect_save_action_in_store(&self) -> (DigestInfo, ActionResult) {
        let mut rx_call_lock = self.rx_call.lock().await;
        match rx_call_lock.recv().await.expect("Could not recieve msg in mpsc") {
            RunningActionManagerCalls::SaveActionInStore(req) => req,
            _ => panic!("Got incorrect call waiting for save_action_in_store"),
        }
    }
...
impl RunningActionsManager for MockRunningActionsManager {
    ...
    async fn save_action_in_store(&self, action_digest: DigestInfo, action_result: ActionResult) -> Result<(), Error> {
        self.tx_call
            .send(RunningActionManagerCalls::SaveActionInStore((
                action_digest,
                action_result,
            )))
            .expect("Could not send request to mpsc");
