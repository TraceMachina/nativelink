...
impl RunningAction for RunningActionImpl {
                    ...
                    let prepare_output_directories = |output_file| {
                        ...
                        async move {
                            ...
                            fs::create_dir_all(full_parent_path).await.err_tip(|| {
                                format!("Error creating output directory {} (file)", full_parent_path.display())
                            })?;
                            Result::<(), Error>::Ok(())
                        }
                    };
    ...
    async fn execute(self: Arc<Self>) -> Result<Arc<Self>, Error> {
        self.metrics().clone().execute.wrap(async move {
                ...
                for entry in output_paths {
                    ...
                    output_path_futures.push(async move {
                        ...
                        if metadata.is_dir() {
                            ...
                            let output_symlink =
                                upload_symlink(&full_path, work_directory)
                                    .await
                                ...
                                Err(e) => {
                                    if e.code != Code::NotFound {
                                        return Err(e).err_tip(|| {
                                            format!("While querying target symlink metadata for {full_path}")
                                        });
                                    }
    ...
    async fn create_and_add_action(
        ...
        self.metrics
            ...
            .wrap(async move {
                ...
                let action_info = self_ref.create_action_info(start_execute, queued_timestamp).await?;
                log::info!("\x1b[0;31mWorker Received Action\x1b[0m: {:?}", action_info);
                let action_id = action_info.unique_qualifier.get_hash();
                let work_directory = self_ref.make_work_directory(&action_id).await?;
                ...
                let running_action = Arc::new(RunningActionImpl::new(
                    ...
                    action_id,
                    work_directory,
                    self_ref.entrypoint_cmd.clone(),
                    action_info,
                    self_ref.clone(),
                ));
                {
                    let mut running_actions = self_ref.running_actions.lock();
