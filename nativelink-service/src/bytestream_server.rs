...
impl Drop for ActiveStreamGuard<'_> {
    fn drop(&mut self) {
        ...
        let timeout = self
            ...
            .map_or(DEFAULT_PERSIST_STREAM_ON_DISCONNECT_TIMEOUT, |config| {
                config.persist_stream_on_disconnect_timeout
            });
...
impl ByteStreamServer {
    pub fn new(
        bytestream_config: &[WithInstanceName<ByteStreamConfig>],
        store_manager: &StoreManager,
    ) -> Result<Self, Error> {
        let mut stores = HashMap::with_capacity(bytestream_config.len());
        for config in bytestream_config {
    ...
    pub fn into_service(self) -> Server<Self> {
        let max_decoding_message_size = self
            .stores
            .values()
            .map(|config| config.max_decoding_message_size)
            .max()
    ...
    fn create_or_join_upload_stream(
        ...
        let (uuid, bytes_received) = match self.active_uploads.lock().entry(uuid) {
            Entry::Occupied(mut entry) => {
                ...
                return Ok(idle_stream.into_active_stream(
                    bytes_received,
                    self,
    ...
    async fn inner_write(
        ...
        let mut active_stream_guard =
            self.create_or_join_upload_stream(uuid, instance_name, digest)?;
...
impl ByteStream for ByteStreamServer {
    ...
    async fn read(
        ...
        let resp = self
            .inner_read(instance_name, digest, read_request)
    ...
    async fn write(
        ...
        self.inner_write(&instance_name, digest, stream)
