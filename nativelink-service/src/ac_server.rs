...
use bytes::BytesMut;
use nativelink_config::cas_server::{AcStoreConfig, NamedConfig};
...
impl AcServer {
    pub fn new(
        config: &[NamedConfig<AcStoreConfig>],
        ...
        for NamedConfig { name, spec } in config {
            let store = store_manager
                ...
                .ok_or_else(|| make_input_err!("'ac_store': '{}' does not exist", spec.ac_store))?;
            stores.insert(
                name.to_string(),
                AcStoreInfo {
                    store,
