        ...
        loop {
            select! {
                ...
                complete_msg = shutdown_rx.recv().fuse() => {
                    event!(Level::WARN, "Shutdown signal received. Shutting down worker...",);
                    let mut grpc_client = self.grpc_client.clone();
                    let worker_id = self.worker_id.clone();
                    let running_actions_manager = self.running_actions_manager.clone();
                    let complete_msg_clone = complete_msg.expect("Failed to receive shutdown message").clone();
                    let shutdown_future = async move {
                        if let Err(e) = grpc_client.going_away(GoingAwayRequest { worker_id }).await {
                            eprintln!("Failed to send GoingAwayRequest: {e}");
                            return Err(e.into());
                        }
