...
pub async fn new_local_worker(
    ...
    let max_cleanup_wait = if config.max_cleanup_wait == 0 {
        DEFAULT_MAX_CLEANUP_WAIT
    } else {
        Duration::from_secs(config.max_cleanup_wait as u64)
    };
    let max_cleanup_backoff = if config.max_cleanup_backoff == 0 {
        DEFAULT_MAX_CLEANUP_BACKOFF
    } else {
        Duration::from_millis(config.max_cleanup_backoff as u64)
    };
