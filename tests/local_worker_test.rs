...
use mock::local_worker_test_utils::{setup_grpc_stream, setup_local_worker, setup_local_worker_with_config};
use mock::mock_running_actions_manager::MockRunningAction;
use native_link::config::cas_server::{LocalWorkerConfig, WorkerProperty};
use native_link::error::{make_err, make_input_err, Code, Error};
use native_link::scheduler::action_messages::{
    ActionInfo, ActionInfoHashKey, ActionResult, ActionStage, ExecutionMetadata,
};
use native_link::scheduler::platform_property_manager::PlatformProperties;
...
use native_link::util::common::{encode_stream_proto, fs, DigestInfo};
use native_link::util::digest_hasher::DigestHasherFunc;
use native_link::worker::local_worker::new_local_worker;
use prost::Message;
    ...
    async fn new_local_worker_creates_work_directory_test() -> Result<(), Box<dyn std::error::Error>> {
        let cas_store = Arc::new(FastSlowStore::new(
            &native_link::config::stores::FastSlowStore {
                ...
                fast: native_link::config::stores::StoreConfig::memory(
                    native_link::config::stores::MemoryStore::default(),
                ),
                slow: native_link::config::stores::StoreConfig::memory(
                    native_link::config::stores::MemoryStore::default(),
                ),
            },
            Arc::new(
                <FilesystemStore>::new(&native_link::config::stores::FilesystemStore {
            ...
            Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default())),
        ));
        let ac_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
