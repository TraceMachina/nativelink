...
use native_link::scheduler::platform_property_manager::{PlatformProperties, PlatformPropertyValue};
mod mock {
    pub(crate) mod scheduler_utils;
}
use mock::scheduler_utils::{make_base_action_info, INSTANCE_NAME};
use native_link::scheduler::simple_scheduler::SimpleScheduler;
use native_link::scheduler::worker::{Worker, WorkerId};
...
mod scheduler_tests {
    ...
    async fn basic_add_action_with_one_worker_test() -> Result<(), Error> {
        ...
        let scheduler = SimpleScheduler::new_with_callback(
            &native_link::config::schedulers::SimpleScheduler::default(),
            || async move {},
        );
    ...
    async fn cacheable_items_join_same_action_queued_test() -> Result<(), Error> {
...

        {
            ...
            expected_action_state.unique_qualifier = action_state1.unique_qualifier.clone();
    ...
    async fn worker_timesout_reschedules_running_job_test() -> Result<(), Error> {
...

        {
            ...
            let action_state = client_rx.borrow_and_update();
            // We now know the name of the action so populate it.
            expected_action_state.unique_qualifier = action_state.unique_qualifier.clone();
            assert_eq!(action_state.as_ref(), &expected_action_state);
        }
