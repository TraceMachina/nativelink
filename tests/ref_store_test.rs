...
mod ref_store_tests {
    ...
    use native_link::store::ref_store::RefStore;
    ...
    fn setup_stores() -> (Arc<StoreManager>, Arc<MemoryStore>, Arc<RefStore>) {
        ...
        let memory_store_owned = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
        store_manager.add_store("foo", memory_store_owned.clone());

        let ref_store_owned = Arc::new(RefStore::new(
            &native_link::config::stores::RefStore {
                name: "foo".to_string(),
            },
