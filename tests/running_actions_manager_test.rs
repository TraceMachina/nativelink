...
use native_link::config::cas_server::EnvironmentSource;
use native_link::error::{make_input_err, Code, Error, ResultExt};
#[cfg_attr(target_family = "windows", allow(unused_imports))]
use native_link::scheduler::action_messages::{
    ActionResult, DirectoryInfo, ExecutionMetadata, FileInfo, NameOrPath, SymlinkInfo,
};
use native_link::store::ac_utils::{compute_digest, get_and_decode_digest, serialize_and_upload_message};
use native_link::store::fast_slow_store::FastSlowStore;
use native_link::store::filesystem_store::FilesystemStore;
use native_link::store::memory_store::MemoryStore;
use native_link::store::Store;
use native_link::util::common::{fs, DigestInfo};
use native_link::util::digest_hasher::DigestHasherFunc;
use native_link::worker::running_actions_manager::{
...
async fn setup_stores() -> Result<
    ...
    Error,
> {
    let fast_config = native_link::config::stores::FilesystemStore {
        content_path: make_temp_path("content_path"),
        temp_path: make_temp_path("temp_path"),
        eviction_policy: None,
        ..Default::default()
    };
    let slow_config = native_link::config::stores::MemoryStore::default();
    ...
    let cas_store = Pin::new(Arc::new(FastSlowStore::new(
        &native_link::config::stores::FastSlowStore {
            fast: native_link::config::stores::StoreConfig::filesystem(fast_config),
            slow: native_link::config::stores::StoreConfig::memory(slow_config),
        },
    ...
    async fn ensure_output_files_full_directories_are_created_no_working_directory_test(
        ...
        let running_actions_manager = Arc::new(RunningActionsManagerImpl::new_with_callbacks(
            RunningActionsManagerArgs {
                ...
                upload_action_result_config: &native_link::config::cas_server::UploadActionResultConfig {
                    upload_ac_results_strategy: native_link::config::cas_server::UploadCacheResultsStrategy::Never,
                    ..Default::default()
                },
...
";
    ...
    async fn caches_results_in_action_cache_store() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let running_actions_manager = Arc::new(RunningActionsManagerImpl::new(RunningActionsManagerArgs {
            ...
            historical_store: Pin::into_inner(cas_store.clone()),
            upload_action_result_config: &native_link::config::cas_server::UploadActionResultConfig {
                upload_ac_results_strategy: native_link::config::cas_server::UploadCacheResultsStrategy::SuccessOnly,
                ..Default::default()
            },
            max_action_timeout: Duration::MAX,
            timeout_handled_externally: false,
    ...
    async fn failed_action_does_not_cache_in_action_cache() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let running_actions_manager = Arc::new(RunningActionsManagerImpl::new(RunningActionsManagerArgs {
            ...
            historical_store: Pin::into_inner(cas_store.clone()),
            upload_action_result_config: &native_link::config::cas_server::UploadActionResultConfig {
                upload_ac_results_strategy: native_link::config::cas_server::UploadCacheResultsStrategy::Everything,
                ..Default::default()
            },
            max_action_timeout: Duration::MAX,
            timeout_handled_externally: false,
        ...
        let running_actions_manager = Arc::new(RunningActionsManagerImpl::new(RunningActionsManagerArgs {
            ...
            upload_action_result_config: &native_link::config::cas_server::UploadActionResultConfig {
                upload_historical_results_strategy: Some(
                    native_link::config::cas_server::UploadCacheResultsStrategy::SuccessOnly,
                ),
    ...
    async fn infra_failure_does_cache_in_historical_results() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let running_actions_manager = Arc::new(RunningActionsManagerImpl::new(RunningActionsManagerArgs {
            ...
            upload_action_result_config: &native_link::config::cas_server::UploadActionResultConfig {
                upload_historical_results_strategy: Some(
                    native_link::config::cas_server::UploadCacheResultsStrategy::FailuresOnly,
                ),
