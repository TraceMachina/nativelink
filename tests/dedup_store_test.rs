...
use native_link::error::{Code, Error, ResultExt};
use native_link::store::dedup_store::DedupStore;
use native_link::store::memory_store::MemoryStore;
use native_link::store::traits::StoreTrait;
use native_link::util::common::DigestInfo;
use rand::rngs::SmallRng;
use rand::{Rng, SeedableRng};

fn make_default_config() -> native_link::config::stores::DedupStore {
    native_link::config::stores::DedupStore {
        index_store: native_link::config::stores::StoreConfig::memory(
            native_link::config::stores::MemoryStore::default(),
        ),
        content_store: native_link::config::stores::StoreConfig::memory(
            native_link::config::stores::MemoryStore::default(),
        ),
        min_size: 8 * 1024,
        normal_size: 32 * 1024,
...
mod dedup_store_tests {
    ...
    async fn simple_round_trip_test() -> Result<(), Error> {
        let store_owned = DedupStore::new(
            &make_default_config(),
            Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default())), // Index store.
            Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default())), // Content store.
        );
    ...
    async fn check_missing_last_chunk_test() -> Result<(), Error> {
        let content_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
        let store_owned = DedupStore::new(
            &make_default_config(),
            Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default())), // Index store.
            content_store.clone(),
        ...
        const LAST_CHUNK_HASH: &str = "7c8608f5b079bef66c45bd67f7d8ede15d2e1830ea38fd8ad4c6de08b6f21a0c";
    ...
    async fn has_checks_content_store() -> Result<(), Error> {
        let index_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
        let content_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore {
            eviction_policy: Some(native_link::config::stores::EvictionPolicy {
                max_count: 10,
                ..Default::default()
            }),
        }));
