...
fn make_stores() -> (Arc<impl StoreTrait>, Arc<impl StoreTrait>, Arc<impl StoreTrait>) {
    let fast_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
    let slow_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
    let fast_slow_store = Arc::new(FastSlowStore::new(
        &native_link::config::stores::FastSlowStore {
            fast: native_link::config::stores::StoreConfig::memory(native_link::config::stores::MemoryStore::default()),
            slow: native_link::config::stores::StoreConfig::memory(native_link::config::stores::MemoryStore::default()),
        },
        fast_store.clone(),
        slow_store.clone(),
    ));
