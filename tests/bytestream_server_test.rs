...
use native_link::util::common::{encode_stream_proto, DigestInfo};
...
async fn make_store_manager() -> Result<Arc<StoreManager>, Error> {
    ...
    store_manager.add_store(
        "main_cas",
        store_factory(
            &native_link::config::stores::StoreConfig::memory(native_link::config::stores::MemoryStore::default()),
            &store_manager,
...
fn make_bytestream_server(store_manager: &StoreManager) -> Result<ByteStreamServer, Error> {
    ByteStreamServer::new(
        &native_link::config::cas_server::ByteStreamConfig {
            cas_stores: hashmap! {
