...
use native_link::store::compression_store::{
...
use native_link::store::traits::{StoreTrait, UploadSizeInfo};
use native_link::util::buf_channel::make_buf_channel_pair;
...
mod compression_store_tests {
    ...
    async fn simple_smoke_test() -> Result<(), Error> {
        let store_owned = CompressionStore::new(
            native_link::config::stores::CompressionStore {
                backend: native_link::config::stores::StoreConfig::memory(
                    native_link::config::stores::MemoryStore::default(),
                ),
                compression_algorithm: native_link::config::stores::CompressionAlgorithm::LZ4(
                    native_link::config::stores::Lz4Config { ..Default::default() },
                ),
            },
    ...
    async fn partial_reads_test() -> Result<(), Error> {
        let store_owned = CompressionStore::new(
            native_link::config::stores::CompressionStore {
                ...
                compression_algorithm: native_link::config::stores::CompressionAlgorithm::LZ4(
                    native_link::config::stores::Lz4Config {
                        block_size: 10,
                        ..Default::default()
                    },
                ),
    ...
    async fn sanity_check_zero_bytes_test() -> Result<(), Error> {
        let inner_store = Arc::new(MemoryStore::new(&native_link::config::stores::MemoryStore::default()));
        let store_owned = CompressionStore::new(
            native_link::config::stores::CompressionStore {
                backend: native_link::config::stores::StoreConfig::memory(
                    native_link::config::stores::MemoryStore::default(),
