...
use native_link::store::filesystem_store::{
...
use native_link::util::buf_channel::{make_buf_channel_pair, DropCloserReadHalf};
use native_link::util::common::{fs, DigestInfo};
use native_link::util::evicting_map::LenEntry;
...
mod filesystem_store_tests {
    ...
    async fn valid_results_after_shutdown_test() -> Result<(), Error> {
        ...
        {
            let store = Box::pin(
                FilesystemStore::<FileEntryImpl>::new(&native_link::config::stores::FilesystemStore {
    ...
    async fn temp_files_get_deleted_on_replace_test() -> Result<(), Error> {
        ...
        let store = Box::pin(
            FilesystemStore::<TestFileEntry<LocalHooks>>::new(&native_link::config::stores::FilesystemStore {
                content_path: content_path.clone(),
                temp_path: temp_path.clone(),
