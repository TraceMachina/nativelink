...
mod filesystem_store_tests {
    ...
    async fn get_file_size_uses_block_size() -> Result<(), Error> {
        ...
        const VALUE_4B: &str = "xabc";

        let digest_1b = DigestInfo::try_new(HASH1, VALUE_1B.len())?;
        let digest_4b = DigestInfo::try_new(HASH2, VALUE_4B.len())?;
        ...
        assert_eq!(short_entry.size_on_disk(), block_size);

        store.as_ref().update_oneshot(digest_4b, VALUE_4B.into()).await?;
        let long_entry = store.as_ref().get_file_entry_for_digest(&digest_4b).await?;
        assert_eq!(long_entry.size_on_disk(), block_size);
        Ok(())
    }
}
