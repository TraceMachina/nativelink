    ...
    async fn verify_sha256_hash_true_suceeds_on_update() -> Result<(), Error> {
        ...
        let store_owned = VerifyStore::new(
            &nativelink_config::stores::VerifyStore {
                ...
                verify_size: false,
                verify_hash: true,
                digest_hash_function: Some(nativelink_config::cas_server::ConfigDigestHashFunction::sha256),
            },
            inner_store.clone(),
        );
        let store = Pin::new(&store_owned);
    ...
    async fn verify_blake3_hash_true_suceeds_on_update() -> Result<(), Error> {
        ...
        let store_owned = VerifyStore::new(
            &nativelink_config::stores::VerifyStore {
                ...
                verify_size: false,
                verify_hash: true,
                digest_hash_function: Some(nativelink_config::cas_server::ConfigDigestHashFunction::blake3),
            },
            inner_store.clone(),
        );
        let store = Pin::new(&store_owned);
