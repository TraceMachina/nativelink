...
mod redis_store_tests {
    ...
    pub async fn new_ext(
        ...
        for cmd in &mock_commands {
            let mock_cmd = match cmd.as_str() {
                "APPEND" => {
                    ...
                    MockCmd::with_values(redis::cmd(cmd).arg(digest).arg(data), Ok(vec![1, 1]))
                }
                "RENAME" => {
        ...
        let redis_store =
            RedisStore::new_with_conn_and_name_generator(connection, mock_name_generator)?;
    ...
    async fn upload_empty_data() -> Result<(), Error> {
        ...
        let digest = DigestInfo::try_new("", 1)?;
        let packed_hash_hex = format!("{}-{}", digest.hash_str(), digest.size_bytes);

        let mock_commands = Some(vec!["RENAME".to_string(), "STRLEN".to_string()]);
        let mock_data = Some(vec![
            TEMP_UUID.to_string(),
            packed_hash_hex.clone(),
            packed_hash_hex,
