...
mod redis_store_tests {
    ...
    #[tokio::test]
    async fn test_local_redis_connection() -> Result<(), Error> {
        let data1 = Bytes::from_static(b"13");
        let data2 = Bytes::from_static(b"14");

        // Assuming Redis is running on localhost with the default port and no auth
        let config = nativelink_config::stores::RedisStore {
            url: "redis://127.0.0.1:6379/".to_string(),
        };
        // Attempt to create a new RedisStore instance
        let store = RedisStore::<MultiplexedConnection>::new(&config)
            .await
            .unwrap();

        let pinned_store: Pin<&RedisStore<MultiplexedConnection>> = Pin::new(&store);
