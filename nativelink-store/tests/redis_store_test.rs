...
impl MockRedisBackend {
    ...
    fn subscribe(&self) -> watch::Receiver<MockCommand> {
        self.rx.clone()
    }
}
...
impl Mocks for MockRedisBackend {
    fn process_command(&self, actual: MockCommand) -> Result<RedisValue, RedisError> {
        self.tx
            .send(actual.clone())
            .expect("the struct should contain at least one receiver");
...
async fn yield_between_sending_packets_in_update() -> Result<(), Error> {
    ...
    tokio::try_join!(
        ...
        async {
            let mut store_rx = mocks.subscribe();
            tx.send(data_p1).await?;
            store_rx
                .wait_for(|cmd| *cmd == first_append)
                .await
                .expect("to receive first append");
