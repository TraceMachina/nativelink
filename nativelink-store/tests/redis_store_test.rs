...
mod redis_store_tests {
    ...
    impl MockRedisConnectionBuilder {
        ...
        fn pipe(mut self, inputs: &[(&str, &[&str], Result<&[redis::Value], RedisError>)]) -> Self {
        ...
        fn cmd(mut self, cmd: &str, args: &[&str], result: Result<&str, RedisError>) -> Self {
    ...
    async fn upload_and_get_data() -> Result<(), Error> {
        ...
        let redis_connection = MockRedisConnectionBuilder::new()
            ...
            .cmd("RENAME", &[TEMP_UUID, &packed_hash_hex], Ok("1"))
            .pipe(&[(
                "STRLEN",
                &[&packed_hash_hex],
        ...
        store =
            RedisStore::new_with_conn_and_name_generator(redis_connection, mock_uuid_generator)?;
