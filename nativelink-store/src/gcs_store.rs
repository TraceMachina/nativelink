...
impl GcsClientWrapper {
    fn new(channel: Channel) -> Self {
        Self {
            inner: StorageClient::new(channel),
        }
    }

    // Unified request handler for common error mapping
    async fn handle_request<T>(
        &mut self,
        operation: impl FnOnce(
            &mut StorageClient<Channel>,
        )
...
impl GcsClient {
    async fn new(
        ...
        // Configure HTTP/2 if not disabled
        if !spec.disable_http2 {
            channel = channel.http2_keep_alive_interval(Duration::from_secs(30));
        }
        ...
        Ok(Self {
            client: Arc::new(RwLock::new(GcsClientWrapper::new(channel))),
                    ...
                    let attempt_result = async {
                        let (upload_res, bind_res) = tokio::join!(
                            async {
                                ...
                                let request_stream =
                                    futures::stream::iter(vec![init_request, data_request]);
                    ...
                    while let Some(response) = stream.next().await {
                        match response {
                            ...
                            Err(e) => {
                                return Err(make_err!(
                                    Code::Aborted,
                                    "Error in upload response stream: {e:?}"
                                ));
                            }
                        }
                    }
