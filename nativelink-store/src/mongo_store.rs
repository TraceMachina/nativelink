...
/// Change stream event types we care about
const CHANGE_STREAM_INSERT: &str = "insert";
const CHANGE_STREAM_UPDATE: &str = "update";
const CHANGE_STREAM_REPLACE: &str = "replace";
const CHANGE_STREAM_DELETE: &str = "delete";
...
impl ExperimentalMongoStore {
    ...
    pub async fn new(mut spec: ExperimentalMongoSpec) -> Result<Arc<Self>, Error> {
        ...
        let mut client_options = ClientOptions::parse(&spec.connection_string)
            .await
            .err_tip(|| "Failed to parse MongoDB connection string")?;
        ...
        if let Some(w) = spec.write_concern_w {
            ...
            write_concern.w(Some(w.into()));
            if let Some(j) = spec.write_concern_j {
                write_concern.journal(j);
            }
            if let Some(timeout) = spec.write_concern_timeout_ms {
                write_concern.w_timeout(Some(Duration::from_millis(timeout)));
            }
        ...
        let client = MongoClient::with_options(client_options)
            .err_tip(|| "Failed to create MongoDB client")?;
    ...
    async fn create_indexes(
        ...
        cas_collection
            ...
            .await
            .err_tip(|| "Failed to create size index on CAS collection")?;
...
impl StoreDriver for ExperimentalMongoStore {
    ...
    async fn list(
        ...
        let mut cursor = self.cas_collection.find(filter, find_options).await
            .err_tip(|| "Failed to create cursor in list")?;

        let mut count = 0u64;
        while let Some(doc) = cursor.try_next().await
            .err_tip(|| "Failed to get next document in list")? {
    ...
    async fn update(
        ...
        if is_zero_digest(key.borrow()) {
            let chunk = reader
                .peek()
                .await
                .err_tip(|| "Failed to peek in ExperimentalMongoStore::update")?;
            if chunk.is_empty() {
                reader
                    .drain()
                    .await
                    .err_tip(|| "Failed to drain in ExperimentalMongoStore::update")?;
                return Ok(());
            }
        }
        ...
        self.cas_collection
            ...
            .await
            .err_tip(|| "Failed to update document in MongoDB")?;
    ...
    async fn get_part(
        ...
        if is_zero_digest(key.borrow()) {
            return writer
                .send_eof()
                .err_tip(|| "Failed to send zero EOF in mongo store get_part");
        }
        ...
        let doc = self.cas_collection
            ...
            .await
            .err_tip(|| "Failed to find document in get_part")?
            .ok_or_else(|| {
                make_err!(
                    Code::NotFound,
        ...
        if offset > data_len {
            return writer
                .send_eof()
                .err_tip(|| "Failed to send EOF in mongo store get_part");
        }
        ...
        if offset < end {
            ...
            for chunk_data in chunk.chunks(self.read_chunk_size) {
                writer
                    ...
                    .await
                    .err_tip(|| "Failed to write data in ExperimentalMongoStore::get_part")?;
            }
        }

        writer
            .send_eof()
            .err_tip(|| "Failed to write EOF in mongo store get_part")
    }
...
impl HealthStatusIndicator for ExperimentalMongoStore {
    ...
    async fn check_health(&self, namespace: Cow<'static, str>) -> HealthStatus {
        match self.database.run_command(doc! { "ping": 1 }, None).await {
            ...
            Err(e) => HealthStatus::Unhealthy(format!("{namespace} - MongoDB connection error: {e}")),
        }
    }
}
                    ...
                    match change_stream_result {
                        Ok(mut change_stream) => {
                            ...
                            while let Some(event_result) = change_stream.next().await {
                                match event_result {
                                    Ok(event) => {
                                        if let Some(op_type) = event.operation_type.as_str() {
                                            match op_type {
                                                CHANGE_STREAM_INSERT | CHANGE_STREAM_UPDATE
                                                | CHANGE_STREAM_REPLACE | CHANGE_STREAM_DELETE => {
...
impl SchedulerStore for ExperimentalMongoStore {
            ...
            + Send,
    {
        ...
        let maybe_index = data.get_indexes()
            .err_tip(|| "Error getting indexes in ExperimentalMongoStore::update_data")?;
        ...
        if <T as SchedulerStoreKeyProvider>::Versioned::VALUE {
            ...
            let data_bytes = data.try_into_bytes()
                .err_tip(|| "Could not convert value to bytes in ExperimentalMongoStore::update_data")?;
            ...
            self.scheduler_collection
                ...
                .await
                .err_tip(|| "Failed to update scheduler document")?;
        ...
        K: SchedulerIndexProvider + SchedulerStoreDecodeTo + Send,
    {
        ...
        self.scheduler_collection
            ...
            .await
            .err_tip(|| "Failed to create scheduler index")?;
        ...
        let cursor = self.scheduler_collection
            ...
            .await
            .err_tip(|| "Failed to create cursor in search_by_index_prefix")?;

        Ok(cursor.map(move |result| {
            let doc = result.err_tip(|| "Error reading document in search_by_index_prefix")?;
            ...
            K::decode(version, data)
                .err_tip(|| "Failed to decode in search_by_index_prefix")
        }))
    }
        ...
        K: SchedulerStoreKeyProvider + SchedulerStoreDecodeTo + Send,
    {
        ...
        let doc = self.scheduler_collection
            .find_one(filter, None)
            .await
            .err_tip(|| "Failed to find document in get_and_decode")?;
        ...
        Ok(Some(K::decode(version, data)
            .err_tip(|| "Failed to decode in get_and_decode")?))
    }
}
