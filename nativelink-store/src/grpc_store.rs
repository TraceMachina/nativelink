    ...
    E: Into<Error> + 'static,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let result = match maybe_message {
            Some(Ok(mut message)) => match ResourceInfo::new(&message.resource_name, true) {
                Ok(mut resource_name) => {
                    resource_name.instance_name = &local_state.instance_name;
                    message.resource_name = resource_name.to_string(true);
                    Some(message)
                }
                Err(err) => {
                    error!("{err:?}");
...
impl Store for GrpcStore {
    ...
    async fn get_part_ref(
        ...
        loop {
            ...
            let message = message_result.err_tip(|| "While fetching message in GrpcStore::get_part()")?;
