...
use azure_core::http::{RequestContent, RetryOptions, StatusCode, Url};
...
pub struct AzureBlobStore<NowFn> {
    // The official SDK clients are not `Clone`, so we share via `Arc` and clone
    // the `Arc` into each retried operation.
    ...
    NowFn: Fn() -> I + Send + Sync + Unpin + 'static,
{
    ...
    fn build_container_client(spec: &ExperimentalAzureSpec) -> Result<BlobContainerClient, Error> {
        ...
        // NativeLink's `Retrier` wraps each operation, so disable the SDK's own
        // retry policy to avoid double-retrying.
