...
/// This is a health check for the filesystem store. It will create a temp file in the temp
/// directory, write to it, flush it, keep it, and then remove it. If any of these steps fail, the
...
impl<Fe: FileEntry> HealthStatusIndicator for FilesystemStore<Fe> {
    async fn check_health(&self) -> HealthStatus {
        ...
        let failed_fn = |failure_action, error| {
            ...
            error!(message);
            HealthStatus::new_failed(self, message.into())
        };
        ...
        match temp_file_with_prefix {
            Ok(mut file) => {
                ...
                let file_path: std::path::PathBuf = match file.keep() {
                    ...
                    Err(err) => return failed_fn("keep", err.into()),
                };
                ...
                match fs::remove_file(file_path.as_path()).await {
                    ...
                    Err(err) => return failed_fn("remove", err.to_std_err()),
                }

                HealthStatus::new_ok(self, "Successfully filesystem store health check".into())
            }

            Err(err) => return failed_fn("create", err),
        }
    }
}
