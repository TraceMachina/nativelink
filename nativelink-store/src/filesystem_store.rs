...
async fn add_files_to_cache<Fe: FileEntry>(
    ...
    async fn read_files(
        ...
        let read_dir = if let Some(folder) = folder {
            format!("{}/{folder}/", shared_context.content_path) // read the current cache
        } else {
            format!("{}/", shared_context.content_path) // read old cache files
        };
        ...
        read_dir_stream
            .map(|dir_entry| async move {
                ...
                let is_file = metadata.is_file(); // we need to filter out folders
    ...
    async fn add_files_to_cache<Fe: FileEntry>(
        ...
        let mut file_infos = read_files(folder, shared_context).await?;
        let file_type = match folder {
            Some(STR_FOLDER) => FileType::String,
            Some(DIGEST_FOLDER) | None => FileType::Digest,
            _ => panic!("Invalid folder type"),
        };

        let path_root = if let Some(folder) = folder {
            format!("{}/{folder}", shared_context.content_path) // read the current cache
        } else {
            shared_context.content_path.to_string() // read old cache files
        };
    ...
    // legaacy cache files
    add_files_to_cache(evicting_map, anchor_time, shared_context, block_size, None).await?;
    // digest cache files
    add_files_to_cache(
        evicting_map,
        anchor_time,
