...
use crate::gcs_client::types::{
    CHUNK_SIZE, DEFAULT_CONTENT_TYPE, GcsObject, INITIAL_UPLOAD_RETRY_DELAY_MS, MAX_UPLOAD_RETRIES,
    MAX_UPLOAD_RETRY_DELAY_MS, ObjectPath, SIMPLE_UPLOAD_THRESHOLD, Timestamp,
};
...
impl GcsClient {
    ...
    pub async fn new(spec: &GcsSpec) -> Result<Self, Error> {
        ...
        // Check https://docs.rs/google-cloud-storage/0.24.0/google_cloud_storage/#automatically
        // for more details.
        ...
        // Trying application default credentials if still not authenticated
        // TODO: This can be removed if we decide not to use the service email and private key
        // for authentication directly and use the application default credentials.
        // For eg: S3Spec can be reused here, if we decide to remove the service email.
        if !auth_success {
            println!("Trying application default credentials");
            tracing::info!("Attempting to use application default credentials");
    ...
    async fn read_and_upload_all(
        ...
        let initial_capacity = std::cmp::min(max_size as usize, 10 * 1024 * 1024);
    ...
    async fn try_resumable_upload(
        ...
        metadata.insert("name".to_string(), object.path.clone());
...
impl GcsOperations for GcsClient {
    ...
    async fn write_object(&self, object: &ObjectPath, content: Vec<u8>) -> Result<(), Error> {
        ...
        // Create media with name
        let media = Media::new(object.path.clone());
