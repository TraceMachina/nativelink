...
mod test {
    ...
    /// String of the `FT.CREATE` command used to create the index template. It
    /// is done this way so we can use it in both const (compile time) functions
    /// and runtime functions. This is a macro because we need to use it in
    /// multiple places that sometimes require the data as different data types
    /// (specifically for rust's `format_args!` macro).
