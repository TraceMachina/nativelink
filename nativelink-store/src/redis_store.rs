...
impl RedisStore<ConnectionManager> {
    ...
    pub async fn new(mut spec: RedisSpec) -> Result<Arc<Self>, Error> {
        ...
        let client = Client::open(format!("{addr}?protocol=resp3")).err_tip_with_code(|e| {
            (
                Code::InvalidArgument,
...
impl<C: ConnectionLike + Clone + 'static + Send + Sync + Unpin> StoreDriver for RedisStore<C> {
    async fn has_with_results(
        ...
        izip!(
            ...
            iter::repeat(client.connection_manager.clone())
        )
        .map(
            |(key, result, local_semaphore, mut local_connection_manager)| async move {
                ...
                let (blob_len, exists) = pipe()
                    ...
                    .query_async::<(u64, bool)>(&mut local_connection_manager)
                    .await
...
impl<C: Psubscribe + Clone + ConnectionLike + Sync> SchedulerStore for RedisStore<C> {
        ...
        K: SchedulerIndexProvider + SchedulerStoreDecodeTo + Send,
    {
        ...
        Ok(stream.map(|result| {
            ...
            //     redis_map
            //         .remove(Value::SimpleString(VERSION_FIELD_NAME.into()))
