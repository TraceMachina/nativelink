...
impl TryFrom<Value> for RedisCursorData {
    ...
    fn try_from(raw_value: Value) -> Result<Self, RedisError> {
        ...
        let results_map = match value.next().unwrap() {
            Value::Map(d) => d,
            other => {
                error!(?other, "Bad data in ft.aggregate, expected map");
                return Err(RedisError::from((
                    ErrorKind::ParseError,
                    "Non map item: {other:?}",
                )));
        ...
        for (raw_key, value) in results_map {
            let key = match raw_key {
                Value::SimpleString(key) => key,
                other => {
                    error!(
                        ?other,
                        "Bad key in ft.aggregate results map, expected string"
                    );
                    return Err(RedisError::from((
                        ErrorKind::ParseError,
                        "Non string item in results map: {other:?}",
                    )));
                }
            };
            match key.as_str() {
                "results" => {
                    let data_ary = match value {
                        Value::Array(v) => v,
                        other => {
                            error!(
                                ?other,
                                "Bad value in ft.aggregate results map, expected array"
                            );
                            return Err(RedisError::from((
                                ErrorKind::ParseError,
                                "Non array item in results map: {other:?}",
                            )));
                        }
                    };

                    output.data.reserve(data_ary.len());
                    for map_data in data_ary {
                ...
                "total_results" => {
                    let total = match value {
                        ...
                        other => {
                            error!(
                                ?other,
                                "Bad value in ft.aggregate total_results map, expected int"
                            );
                            return Err(RedisError::from((
                                ErrorKind::ParseError,
                                "Non int item in total_results map: {other:?}",
                            )));
                        }
                    };
                    output.total = total;
                }
                "attributes" | "format" | "warning" => {} // known but ignored fields
                other => {
                    error!(?other, "Bad key in ft.aggregate results map");
                    return Err(RedisError::from((
                        ErrorKind::ParseError,
                        "Bad key in results map: {other:?}",
                    )));
                }
            };
        }
