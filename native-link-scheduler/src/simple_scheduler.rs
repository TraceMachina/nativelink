...
impl SimpleSchedulerImpl {
    ...
    fn drain_worker(&mut self, worker_id: WorkerId) -> Result<(), Error> {
        if let Some(worker) = self.workers.workers.get_mut(&worker_id) {
            self.metrics.workers_drained.inc();
            worker.is_draining = true;
        } else {
            return Err(make_err!(Code::Internal, "Worker {} doesn't exist in the pool", worker_id));
        }
        // Note: Calling this many time is very cheap, it'll only trigger `do_try_match` once.
        self.tasks_or_workers_change_notify.notify_one();
        Ok(())
...
impl SimpleScheduler {
    ...
    /// Checks to see if the worker can accept worker. Should only be used in unit tests.
    pub fn can_worker_accept_worker_for_test(&self, worker_id: &WorkerId) -> Result<bool, Error> {
...
impl WorkerScheduler for SimpleScheduler {
    ...
    async fn drain_worker(&self, worker_id: WorkerId) -> Result<(), Error> {
        let mut inner = self.get_inner_lock();
        inner.drain_worker(worker_id)
    }
